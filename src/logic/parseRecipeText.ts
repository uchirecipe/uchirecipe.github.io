...
// 2026-08-22 便IX: 「下準備」を「下ごしらえ」と同じ節の見出しとして足した。cotta実測では
// 材料欄と作り方の間に「下準備」の節があり、見出し語として知らないために**材料の1行**として
// 保存されていた（買い物メモにも「下準備」が並んだ）。中身（「薄力粉をふるっておく。」等）は
// 従来どおり手順として残る。
const STEP_HEADER = /^[【\[（(◆■□●○☆★♪#＊*\s]*(作り方|つくり方|作りかた|手順|調理手順|下ごしらえ|下準備)/
...
const EXACT_GOMI = new Set([
  ...
  // 2026-08-22 便IX（cotta実測）。行全体がこの見出し語だけのときに限って落とす。
  // 貼り付け経路では下の PROMO_BLOCK_HEADING が先に見つけて「見出し＋直後の1行」で落とすので、
  // ここに置く効き目はURL取り込み経路（isImportGomiLine）側＝見出し1行だけを落とす分になる
  'おすすめのアイテム', '使用する道具',
])

/**
 * 2026-08-22 便IX: 材料欄の途中に差し込まれる「宣伝・道具のブロック」の見出し。
 *
 * cotta「基本のシュークリームのレシピ」実測では、材料の並びの途中に
 *   おすすめのアイテム
 *   cotta 北海道産薄力粉 シュクレ 2.5kg
 * という2行が3か所入っており、**見出しも売り物も材料として保存されていた**
 * （買い物メモにもそのまま並んだ）。同じサイトの「基本のマドレーヌ」では
 *   使用する道具
 *   マドレーヌ型
 * が同じ形で入る。
 *
 * 売り物の行（「cotta 北海道産薄力粉 シュクレ 2.5kg」）は名前に「薄力粉」を含み、分量（2.5kg）も
 * 持つので、その行だけを見て材料と見分ける手がかりが無い。**見出しの直後にある**ことが
 * 唯一の決め手なので、落とすのは「この見出し」と「その直後の1行」だけに絞る。
 * 実測3か所すべてで、この見出しの下に並ぶのは商品1件だけだった。
 *
 * 巻き込み防止:
 * - 落とすのは直後の**1行だけ**（「次の見出しまで全部」にすると、間に本当の材料が入った
 *   ページで材料が黙って消える）。
 * - 直後が見出し行（材料・作り方・下準備・コツ等）のときは何も落とさない。
 */
const PROMO_BLOCK_HEADING = /^(?:おすすめのアイテム|使用する道具)$/
const PREFIX_GOMI = [
...
/**
 * うちレシピの共有テキストの末尾に入る、**受け取った人への案内行**（2026-09-07 便NM）。
 * 出るのは「この文章をうちレシピに貼ると、レシピとして取り込めます」
 * （文言は `src/i18n/ja.ts` の `share.textImportInvite`）。
 *
 * 送った人が書いた材料でも手順でもないので、末尾の入口URL（URL_ONLY_LINE）・ハッシュタグと
 * **同じ扱いで落とす**。落とさないと、共有テキストを貼り付けた人のレシピに、この案内が
 * 最後の手順として残る（往復するたびに1件ずつ増える）。
 *
 * 当てるのは**行全体がこの形のときだけ**で、決め手は「うちレシピ」＋「貼る」の並び。
 * 案内の言い回しを変えても効き続けるように、前後は字数だけで見る。
 * 文言と食い違ったら share往復のテスト（`scripts/tests/settings-misc.mjs`）が赤になる。
 */
const APP_IMPORT_INVITE_LINE = /^.{0,16}うちレシピ.{0,12}[にへ]貼[るり].{0,24}$/
/**
 * 行全体が「動画を見てください」という案内だけの手順（2026-08-22 便JJ）。
 *
 * オーナー原文（丸ごと無限ピーマン）:「手順１がいきなり『動画をご覧ください』。これは省けないのか」
 * 実測（リュウジのバズレシピ・文章から取り込み）: 手順の1件目が
 * 「詳しい作り方は動画をご覧ください。」だけで、作り方が1文字も書かれていない。
 * 読んでも料理が1歩も進まない行なので落とす。
 *
 * **行全体がこの形のときだけ**落とす。「焼き加減は動画をご覧ください」のように前後に
 * 作り方が書いてある行は残す（手順が黙って消えるほうが実害が大きい）。
 */
...
function pass1(lines: string[]): string[] {
  ...
  while (i < lines.length) {
    ...
    // 便IX: 宣伝・道具のブロック(「おすすめのアイテム」+ 売り物1行)は、見出しと直後の1行を
    // 対にして落とす。直後が見出し行なら見出しだけを落とす(本当の材料を巻き込まないため)
    if (PROMO_BLOCK_HEADING.test(line.trim())) {
      i++
      while (i < lines.length && lines[i] === '') i++
      if (i < lines.length && classifyHeader(lines[i]) == null) i++
      continue
    }
