 ...
 *
 * ## どの器具を「自分で知らせる器具」とするか
 * 入れるのは**時間を指定して動かし、終わると自分で止まって音で知らせる**器具だけ:
 *   - 電子レンジ（オーブンを含む。`stepAppliance` は家庭で多いオーブンレンジを1台と読むため
 *     同じ 'microwave' に入る。オーブンにも必ずタイマーが付いている）
 *   - トースター（オーブントースターのつまみは時間で回し、終わると切れて鳴る）
 * 入れないもの:
 *   - **コンロ**（「沸くまで」「20分煮る」は器具が何も知らせない＝アプリのタイマーが要る）
 *   - **魚焼きグリル**（タイマーの付いていない機種があり、付いていても消し忘れは焦げに直結する。
 *     知らせてくれると決めつけて外すと、外れたときの被害が大きいほうに倒れる）
 *   - **炊飯器**（`stepAppliance` が数えない器具＝待ちの分数も持たないので、そもそもここへ来ない）
 *
 * 迷ったら**残す**側に倒す（ボタンが1つ余分に出るだけの害と、火を見張る手立てが消える害は釣り合わない）。
 *
 * ## 台所の設定を見る理由
 * その器具を**持っていない**家では、`stepApplianceFor` がその工程をコンロに読み替える。
 * 読み替わった先はアプリのタイマーが要るので、設定どおりボタンを残す。
 *
 * 段取りの計算（分数・器具の占有）はここでは一切変えない＝表示するボタンだけを決める。
 */
export function waitSignaledByAppliance(
  item: Pick<TimelineItem, 'kind' | 'longRest' | 'waitMinutes' | 'text'> & {
    addedByNavi?: boolean
  },
  kitchen: KitchenEquipment = DEFAULT_KITCHEN,
): ApplianceKey | null {
  if (!showsWaitTimerButton(item)) return null
  // ナビが足した工程（湯を沸かす・沸くのを待つ）はコンロの待ちなので、必ずアプリのタイマー
  if (item.addedByNavi) return null
  const key = stepApplianceFor(stepMainText(item.text ?? ''), kitchen)
  return key === 'microwave' || key === 'toaster' ? key : null
}
