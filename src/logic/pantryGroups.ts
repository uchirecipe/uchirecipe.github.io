...
const GROUP_LABELS: Record<PantryGroupKey, string[]> = {
  ...
  vegetable: [
    ...
    // 2026-09-01 便MT（マッシュルーム08031の追加に伴う分類）
    'マッシュルーム',
    'なめこ', '三つ葉', '切り干し大根', 'とうもろこし', '赤パプリカ', 'トマト缶',
    // 2026-08-25 便KP（キムチは八訂でも「はくさい 漬物」＝野菜の並び）
    '豆苗', 'サラダ菜', 'グリーンリーフ', '菜の花', '香菜', 'キムチ',
  ],
  ...
  soyEgg: [
    ...
    // 2026-08-23 便KF
    '大豆（水煮）',
    // 2026-08-25 便KY（八訂で別々に収載されている「◯◯チーズ」「無塩バター」と、
    // 名前に「豆腐」「たまご」が入っているだけで別の食品に寄っていた2品）
    '粉チーズ', 'クリームチーズ', 'マスカルポーネ', '無塩バター', '玉子豆腐',
    // 2026-08-26 便LB（八訂のゴーダで代表するようにした「ピザ用チーズ」。棚は他のチーズと同じ）
    'ピザ用チーズ',
    // 2026-08-26 便LD（八訂の（チーズ類）15品をすべて持つようにした残り10品。棚は他のチーズと同じ）
    'モッツァレラ', 'チェダー', 'カマンベール', 'カッテージチーズ', 'ブルーチーズ',
    'リコッタ', 'エダム', 'エメンタール', 'やぎのチーズ', 'チーズスプレッド',
  ],
  ...
  seasoning: [
    ...
    '減塩しょうゆ', '減塩みそ',
    // 2026-08-25 便KP
    '米油', '三温糖', '練りからし', '粒マスタード', 'キムチの素',
    // 2026-08-25 便KW（オーナー申し送り3「味噌は種類で価格も塩分量も異なります」）
    'だし入りみそ', '麦みそ', '豆みそ',
    // 2026-08-25 便KX（八訂の即席みそ 17050）
    'みそ汁の素',
    // 2026-08-25 便KY
    'だししょうゆ',
    // 2026-08-26 便LB（八訂のぶどう酒 白16010／赤16011。棚は「酒」「みりん」と同じ）
    '白ワイン', '赤ワイン',
  ],
  ...
  other: [
    ...
    'こんにゃく', 'しらたき',
    // 2026-08-25 便KY（ごま豆腐は八訂ではでん粉製品で、大豆の豆腐とは別の並び。
    // アーモンドはナッツで、粉にした「アーモンドプードル」も同じ棚）
    'ごま豆腐', 'アーモンド',
  ],
}
