 ...
 * 共通のレシピカードの「標準」に寄せた。当時の献立の「日」の「最近作ったもの」
 * （旧 components/RecentCookedList）と**同じ役目・同じ形**にそろえたもの。
 * 2026-09-06 便NH: その「最近作ったもの」はレシピの横スクロールの棚（RecipeShelf）に
 * 変わったので、**記録そのものを縦に全部見られるのはこの画面だけ**になった。
...
/*
 * 呼び出し元へ戻すための行き先（2026-08-02 便DE-11・オーナー指示）は
 * logic/backLink.ts の mealPlanTabBackPath が持つ（2026-08-28 便MA が、
 * 献立テンプレートの画面と同じ受け渡しを2か所に書き写さないよう1か所へ移した）。
...
export default function HistoryPage() {
  ...
  const backTarget = mealPlanTabBackPath(searchParams.get('back'))
