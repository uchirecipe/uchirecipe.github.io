 ...
 */
export const PRICE_DEFAULTS_VERSION = 13
...
export const PRICE_DEFAULTS: PriceDefaultItem[] = [
  ...
  // ・減塩みそ: 実売はマルコメ「料亭の味 だし入り 減塩 750g」の店舗別購入価格(ものログ)で
  //   199〜498円・よく出る帯が298〜329円＝中央298円/750g。大さじ1(18g)なら7.2円になるが、
  //   **既存の「みそ 11円/大さじ1」と同じ水準に置く**（青ねぎと同じ考え方＝新しい相場を作らない）。
  //   既存のみその11円は2026-07-21に別ソース（全国平均425円/750g等）から出した値で、
  //   同じ売り場の同じ帯の商品なのに「減塩のほうが3割安い」と出るのは実感と逆になるため。
  //   みそ全体の帯を実売で測り直すのは別の便の宿題（そのとき2件そろえて見直す）
  ...
  // ・赤みそ: 実売はイチビキ「本赤だし750g」アミカ税込356円の1件だけで、しかもこれは赤だしみそ
  //   （豆みそ入り）で成分表の「赤色辛みそ」とは少し違う商品。大さじ1(18g)換算で8.5円と
  //   既存の「みそ 11円/大さじ1」と同じ帯なので、**新しい相場を作らず既存のみそと同額に置く**
  //   （青ねぎ・減塩みそと同じ考え方）
