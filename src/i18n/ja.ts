...
export const ja = {
  ...
  mealPlan: {
    ...
    monthSummaryEstimateNote: '食材の目安価格や食品成分表をもとに自動計算しためやすです',
    ...
    monthLockedDescription:
      'カレンダーで先の予定まで組み立てられます。作った日は写真付きの記録が残り、月ごとの食費や栄養のめやすもまとめて振り返れます。月の画面からは、予定をそのまま追加・変更でき、その日のメモ（外食・実家など）や、未定の日にまとめて提案、1か月分の献立表の印刷・画像保存も使えます。過ぎた日は作った記録だけが残ります（作らなかった予定は表示しません）。',
    ...
    purposeHint:
      '「まとめて献立を立てる」と「おまかせで提案」が、同じ枠を数回引き直して、選んだ目的に沿う組み合わせを採ります。ふだんの提案の決まりごと（同じ日はジャンルをそろえる・同じ料理を続けない）はそのままです。数字はどれもめやすです',
    ...
    purposeReviewNote:
      '数えたのは、この月で数字が出た日だけです（献立も記録も無い日は数えていません）。数字はめやすで、多い少ないの良し悪しは判定していません',
  ...
  settings: {
    ...
    unlockAccuracyNotice:
      '栄養と食費の数字は、材料と分量から計算しためやすです。調理による変化は反映していません。治療中の方・妊娠中の方の食事管理には使えません。',
    ...
    proActivatedFeatures: [
      { label: 'レシピの登録数の上限なし', hint: '50件を超えて登録できます' },
      {
        ...
        label: '栄養価の8項目表示',
        hint: 'レシピを開いて「栄養価のめやす」をタップすると、たんぱく質・脂質・炭水化物・食物繊維・鉄・カルシウム・塩分相当量まで出ます',
        to: '/recipes',
        linkLabel: 'レシピ一覧を開く',
      },
      {
  ...
  share: {
    ...
    lineNutritionKcalOnly: '1食あたり 約{kcal}kcal（めやす）',
    ...
    lineNutritionKcalOnlyPartial: '1食あたり 約{kcal}kcal（めやす・一部の材料を除く）',
  },
  form: {
    newTitle: 'レシピを登録',
    editTitle: 'レシピを編集',
  ...
  nutrition: {
    ...
    proNutrientHighlight: 'Pro版では、たんぱく質・脂質・炭水化物・食物繊維・鉄・カルシウム・塩分相当量のめやすも表示されます。',
   ...
   *  1. 見出しには必ず「めやす」を含める。
   *  2. **不足・過多を断定しない**。「◯g／めやす7.5g」のように数値を並置するだけにとどめ、
   *     「足りません」「摂りすぎです」とは書かない。色でも善悪を表さない。
  ...
  nutritionBalance: {
    ...
    dayTitlePlan: 'この日の献立ぶん（1人分）',
    dayTitleActual: 'この日の作った記録ぶん（1人分）',
    weekTitle: 'この週の献立ぶん（1人分）',
    ...
    dayToggleExpand: 'この日（{d}）の栄養のめやすを詳しく見る',
    dayToggleCollapse: 'この日（{d}）の栄養のめやすを閉じる',
    weekToggleExpand: 'この週の栄養のめやすを詳しく見る',
    weekToggleCollapse: 'この週の栄養のめやすを閉じる',
    vegetableLabel: '野菜',
    // めやすとの並置（docs/60 §1-3-1「1日のめやすとくらべる」）。断定はせず数値を並べるだけ
    guideTitleDay: '1日のめやすとくらべる',
    guideTitleDays: '{n}日ぶんのめやすとくらべる',
    // 第3段（設定「めやすの基準」）を作るまでは男女どちらか分からないため両方を併記する
    // （docs/60 §7 未決#5＝(b)。片方に丸めると「自分の値ではない数字」を出すことになる）
    guideSaltRow: '塩分 {v}g　／　めやす 男性 {male}g・女性 {female}g',
    guideVegetableRow: '野菜 {v}g　／　めやす {guide}g',
    // 週まとめのめやすは「1日のめやす×数えた日数」。7日固定で掛けない理由も画面に書く
    guideDaysNote: 'めやすは1日ぶんの数値を、献立や記録がある{n}日ぶんに伸ばした数です',
    // 便BY/NUT-01と同じ作法: 折りたたんだ1行のままでも「この数字は下振れしている」と分かるように、
    // 計算できなかった料理の件数を見出し行に出す(展開しないと分からない状態にしない)
    gapBadge: '⚠ 計算できない料理 {n}品',
    // docs/60 §5-1/5-2: 合計が下振れしていると分かっているものを「めやす内」と見せない
    noCompareDay: 'この日は栄養を計算できない料理があるため、めやすとのくらべ方は出していません',
    noCompareRange: 'この期間は栄養を計算できない料理が多いため、めやすとのくらべ方は出していません',
    ...
    registeredOnlyMealNote:
      '3食のうち夕食だけを登録している場合は、その1食ぶんの合計を1日のめやすと並べています。',
