...
export const ja = {
  ...
  mealPlan: {
    ...
    // 週タブ「この食事の今週分を空にする」(便U-4 Fable設計。「朝のみ削除したい」への回答)。
    // 2026-07-29 便CD/MP-16: 「帯」という語が通じないという指摘を受け「食事」に言い換えた
    clearWeekSlotTitle: 'この食事の今週分を空にする',
    ...
    monthLockedDescription:
      'カレンダーで先の予定まで組み立てられます。作った日は写真付きの記録が残り、月ごとの食費や栄養のめやすもまとめて振り返れます。',
    ...
    // 2026-07-29 便CD/MP-01: どちらからどちらへ反映されるのか(向き)が一読で掴めず、
    // 逆だと勘違いしそうという指摘(2体)への文言修正。何が変わり何が変わらないかを両方書く
    planMismatchNotice: '今日の献立と今週の予定が食い違っています',
    // 2026-07-29 便CB-1: 以前は料理の種類を見ずに必ず「主菜」として登録し、副菜を押すと
    // その日の主菜が消えていた(便CD報告の不具合)。主菜になる料理は主菜・副菜になる料理は副菜として
    // 入る挙動に直したので、案内文もそのとおりに書き直す。何が変わらないか(今日の献立)も従来どおり明記する
    planMismatchDescription: '下の品は「今日の献立」にだけあります。時間帯を押すと、その品を今週の予定に登録します（主菜になる料理は主菜、副菜になる料理は副菜として入り、今ある献立は消えません）。今日の献立は変わりません',
  ...
  settings: {
    ...
    proDescription: 'レシピの登録数を無制限にしたり、栄養価の8項目表示・栄養価での並び替え・月間の献立表・並行調理ナビなど、Pro向けの機能が使えるようになります。',
    ...
    proActivatedFeatures: [
      ...
      {
        label: '栄養価の8項目表示',
        hint: 'レシピを開いて「栄養価のめやす」をタップすると、たんぱく質・脂質・炭水化物・食物繊維・鉄・カルシウムまで出ます',
        to: '/recipes',
        linkLabel: 'レシピ一覧を開く',
      },
      {
      ...
      { label: '月間ビュー', hint: '献立タブの「週・月」の切り替えから見られます', to: '/meal-plan', linkLabel: '献立を開く' },
  ...
  search: {
    ...
    sortNutritionGate: '栄養価で探す（Pro機能）',
  ...
  urlImport: {
    ...
    confirmReplace:
      '入力済みの{items}は、読み込んだ内容に置き換わって消えます。人数分・調理時間・参照元URLも読み込んだ内容に置き換わります。料理名・ひとこと説明・メモはそのまま残ります。',
  ...
  share: {
    ...
    stepsHeading: '【作り方】',
    dialogTitle: 'シェアする内容',
    alwaysIncluded: '料理名・食数・材料（最初の8件）・作り方は常に入ります',
    ...
    lineNutritionPartial: '1食あたり 約{kcal}kcal・塩分 約{salt}g（めやす・一部の材料を除く）',
  },
  form: {
    newTitle: 'レシピを登録',
    editTitle: 'レシピを編集',
    ...
    freeLimitBlocked: '無料版の登録上限（50件）に達しました。上限の解除はPro版で提供しています。お困りの場合は設定画面の「ご意見箱」からお声をお寄せください（今までのレシピはそのまま使えます）',
  ...
  nutrition: {
    ...
    excludedDirectionNote: '含めていない材料の分は合計に入っていないため、実際の値はこのめやすより大きくなります。',
    ...
    sourceCommercialNote: '※ 成分表に収載が無い食材（市販の加工食品など）だけ、市販品の栄養成分表示を参考にした概算です',
    // 2026-07-28 便BY/NUT-05: 「めやす」がどのくらいの精度なのかを一言で。
    // 幅の数値は根拠を出せないため書かず、ぶれる理由を書く
    precisionNote: '※ 同じ食材でも品種・大きさ・部位で成分は変わるため、数値は幅のあるめやすです。',
