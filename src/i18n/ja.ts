...
export const ja = {
  ...
  settings: {
    ...
    backupImportReplaceConfirm:
      '今のレシピ{r}件・作った記録{c}件・価格{p}件が消え、読み込むファイルの内容に置き換えます。設定（NG食材・テーマ・週の食費予算など）もファイルの内容になります。\nPro版の解錠コードは残ります。\n置き換える前のデータはアプリの中に控えを取るので、この設定画面を開いている間は「元に戻す」で戻せます（別の画面へ移ると戻せなくなります）。\nよろしいですか？',
    ...
    replaceUndoMessage:
      '置き換える前のデータの控えを取りました。この設定画面を離れると「元に戻す」は使えなくなります',
    ...
    replaceSnapshotFailed:
      '置き換える前のデータの控えは取れませんでした（端末の空き容量が足りない可能性があります）。今回は「元に戻す」で戻せません',
    ...
    moveGuideStep3: '③新しい端末でアプリを開き「読み込む（今のデータと置き換え）」でそのファイルを選ぶ',
    moveGuideStep4:
    ...
    moveGuideNote:
      '③は新しい端末の中身を今のファイルの内容に置き換えます。新しい端末で先にレシピを登録していた場合は消えるのでご注意ください。今の端末の初期化・下取りは、新しい端末でレシピと「作った記録」が見えることを確かめてからにしてください',
