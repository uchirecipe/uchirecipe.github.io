 ...
 *
 * **音声で受けるのは、間違っても戻せる操作だけ**（次へ／戻って／最初の手順へ／読み上げ／
 * 読み上げストップ／ストップ／再開／タイマー）。
...
export interface VoiceCommandActions {
  ...
  onPrev: () => void
  /**
   * 「最初の手順へ」＝先頭の手順に戻る（2026-08-15 便GS・オーナー実機
   * 「ボタンと同じ表記にも対応したい」）。並行調理ナビの調理中モードの左上にあるボタンと
   * 同じ働きで、1品の調理中モードでは手順1に戻る。**動くのはカーソルだけ**で手順は消えない
   * ＝言い直せば戻せるので、声で受けてよい（docs/69「音声の規律」）。
   */
  onFirst: () => void
   ...
   * 片方優先するならタイマー」）。
   * **「ストップ」単独はタイマー（onStop）のまま**で、こちらは「読み上げ」の語と
   * 一緒に言われたときにだけ呼ばれる＝オーナーの言う「片方優先」がタイマー側に残る。
   */
  onStopSpeech: () => void
  /**
...
/**
 * 音声コマンドの聞き取り。「次へ」「戻って」「最初の手順へ」「読み上げ」「読み上げストップ」
 * 「ストップ」「再開」「タイマー」と、色を受け取る画面でだけ色（「青」「緑」「ピンク」）を受ける。
 *
  ...
  useEffect(() => {
    ...
    recognition.onresult = (event) => {
      ...
      if (command === 'next') {
        ...
        feedback()
        current.onRepeat()
      } else if (command === 'readStop') {
        // 読み上げだけを止める（2026-08-15 便GS）。タイマーには触らない
        feedback()
        current.onStopSpeech()
