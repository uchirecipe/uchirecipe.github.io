...
type Props = {
  ...
  /** 朝食・昼食・夕食を決めずに選んだ（今日の献立へ直接・今週の予定には入れない） */
  onPickUndecided: () => void
  onClose: () => void
  /**
   * 窓の見出し（任意）。省略するとレシピ1品ぶんの ja.detail.todaySlotDialogTitle。
 ...
 * レシピ詳細のボタン押下で開き、「朝食・昼食・夕食のどれに入れますか？」として
 * [朝食] [昼食] [夕食] と、どれも決めずに入れる選択肢を出す。
 *
 * 2026-08-19 便HT（オーナー原文「『食事』だと意味が分かりづらい」）: 見出しと
 * 「決めずに入れる」ボタンの中で、選択肢（朝食・昼食・夕食）を名指しするようにした。
 * 窓の作り・選択肢の数・押す回数は変えていない（文言だけ）。
