...
export interface Recipe {
   ...
   * 画面の欄名は 2026-09-01 裁定★3で「検索キーワード」→「検索用の言葉」に改名（キーはそのまま）。
   */
  keywords?: string[]
  /**
   * 「1食」に分けて食べる品ではない（2026-08-25 便KS・④）。
   *
   * オーナー原文「１食あたり７０円とあるが、出汁だけで１食とは言わない。ここだけ
   * １食あたり表示なしにするのも変？」。だしのとり方のように、でき上がりを何回かに分けて
   * 使う品は「1食あたり◯円」で割っても意味を成さない。true の品は、レシピ詳細の金額を
   * 「でき上がり全体で 約◯円」と1つだけ出す（1食あたりの行は出さない）。
   *
   * **料理名での特別扱い（「だしのとり方」なら〜）にはしない**: だし・下ごしらえ・
   * 作り置きのもとは今後も増えうるので、レシピ側が持つ印にする。
   * 任意項目なので既存データ・既存のバックアップはそのまま読める（Dexieの版番号も上げない＝
   * 索引を張るフィールドではないため。db/db.ts の版は索引が変わるときだけ上げる）。
   */
  wholeBatch?: boolean
...
export type RecipeInput = Pick<
  ...
  | 'suitableFor'
  | 'keywords'
  // 「1食」に分けて食べる品ではない印（2026-08-25 便KS・④）。同梱レシピの原稿が持つので
  // RecipeInput にも含める。登録画面には出さない（いまは同梱の「だしのとり方」だけが持つ）
  | 'wholeBatch'
>
