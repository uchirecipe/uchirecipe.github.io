...
export interface Settings {
  ...
  /**
   * レシピに添える注意を表示しない（任意・2026-08-22 便JH）。
   * 未設定/false＝表示する（既定）。true＝どの画面にも出さない。
   *
   * 取り込んだレシピ・自分で登録したレシピに、材料と手順の組み合わせから分かる注意
   * （電子レンジと卵の破裂など。logic/safetyNotes.ts）を添える仕組みの入り切り。
   * **レシピのデータには何も書き込まない**ので、切っても入れても中身は変わらない。
   * 「表示しない」だけを持つ形にしてあるので、既存の端末は何もしなくても表示される。
   * 任意項目なのでスキーマ変更・マイグレーション不要。
   */
  safetyNotesOff?: boolean
  /**
   * 絞り込みのタグとして登録した検索の言葉（2026-08-19 便HZ・②・任意項目）。
   * 押すとその言葉での検索が戻る。**レシピのデータには一切書き込まない**。
   * 登録した順に並べる（logic/tagRegister.ts）
   */
  savedSearches?: string[]
  /**
   * 以前の版（2026-08-19 便HU・⑭）が、登録と同時にレシピ本体へ書き込んだタグの控え
   * （任意項目）。便HZでレシピへの書き込みは廃止したが、すでに書き込まれたタグを
   * 黙って消すとデータを失うので、**外す道を残すため**にこの控えは残してある
   * （絞り込みパネルの「以前の版でレシピに書き込まれたタグ」がここを読む）。
   * 新規に増えることはなく、外し終えたぶんから減る
   */
  keywordTags?: string[]
  ...
  /**
   * 献立の「現在の条件」で選んでいる料理のジャンル（任意・2026-08-22 便IY・オーナー原文
   * 「週献立は、「料理のジャンル」は複数選択のほうがいいかも。１つしか選べないと、
   * １週間中華だけ、という献立しか組めない。全てを選ぶと、中華は入れたくないけど和洋食は
   * 混在させたい、ができない。」）。
   *
   * 未設定＝3つとも選んだ状態（指定なし＝全部から選ぶ。この項目が無かった頃と同じ提案）。
   * 覚えておくのは、「うちは中華を作らない」が年単位で続く家庭の好みだから
   * （planPurpose と同じ理由。開くたびに選び直させない）。
   * **読み出しは必ず logic/mealPlan.ts の normalizePlanGenres を通す**＝1つだけ選んでいた頃の
   * 保存値も1件として読み、知らないジャンル名・壊れた値でも候補を0件にしない。
   * 任意項目なのでスキーマ変更・マイグレーション不要。
   */
