    ...
    try {
      ...
      const s2Measure = async () => {
        ...
        return await s2Page.evaluate(() => {
          ...
          // 数えるのは**一覧のカード**だけ。「最近作っていないレシピ」の区画(2026-09-05 便ND)を
          // 混ぜると「ふだん⇔選択中」の枚数の比較が区画のぶんだけ食い違う。
  ...
  currentCheck = 'CARDUNIFY-01'
  {
    ...
    try {
      ...
      /** 先頭のカード1枚の形を測る。写真の有無で中身が変わらないよう、絵の枠(正方形の箱)を見る。
          ...
          コンテナ名ではなく区画の目印で除く。2026-09-06 便NH: 棚は献立の日タブへ移った=
          いまは空振りだが、出戻りに備えて残す) */
      const cuShape = () =>
        cuPage.evaluate(() => {
      ...
      // 下のrecipe-shelfの除外(2026-09-06 便NHで棚は日タブへ): この掴みは一覧で走るので
      // いまは空振りだが、この節の続きは日タブ(棚3段が実在)に着くため、掴みを日タブ側へ
      // 動かした瞬間に要る除外になる。消さないこと。
