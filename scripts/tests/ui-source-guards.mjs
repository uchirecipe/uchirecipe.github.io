 ...
 */
{
  ...
  const OPEN_TO_REFINE = {
    ...
    // --- 並行調理ナビ ---
    // 旧 'ja.cookNavi.ingredientsServings'（材料の一覧の見出しに出る人数。押すのは開閉だけで
    // 操作ではない）は 2026-09-01 便MW で外した: 見出しが logic/servingsUnit.ts 経由
    // （人分/個分の出し分け）になり、キーの字面がソースに出なくなったため掃く網に掛からない。
    // 「押すのは開閉だけ」という設計そのものは変わっていない
    /**
