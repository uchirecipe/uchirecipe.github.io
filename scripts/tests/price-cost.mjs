...
// 原価が過大(サラダ油「適量」=1Lボトル400円)・過小(鶏むね肉「1枚」=100g分90円)の両方向に壊れていた ----------
{
  ...
  //       (出典つき)へ書き換えて按分できるようにしたぶんの回帰固定。価格(円)は据え置き。
  {
    ...
    eq('生しいたけの価格は280円（便EYの単位の直しでは動かしていない。便LFが実勢に合わせた）', byName.get('生しいたけ')?.pricePerUnit, 280)
    ...
    eq(
      'パック按分: 生しいたけ2枚は6枚280円のマスタから93円(従来は1パック満額。便LFの前は33円)',
      estimateIngredientYen({ name: '生しいたけ', amount: '2', unit: '枚' }, idx)?.yen,
      93,
    )
    eq(
      'パック按分: 生しいたけ5枚は233円(従来は1パック満額。便LFの前は83円)',
    ...
    eq(
      'レタス4枚は72円(マスタ1個180円からグラム換算で按分。便LFで150→180円にする前は60円)',
  ...
  // (8) 同梱109品の合計原価のピン留め(この数字が動いたら按分の前提が変わったということ)
  {
    ...
    // 2026-08-26 便LF 第5弾（油）で39,777→40,051円(+274円)。直したのは3件
    ...
    eq('同梱109品の概算食費の合計(便LF第5弾後。便LF第4弾後は39,777円/便LF第3弾後は38,121円/便LF第2弾後は37,802円/便LF第1弾後は37,015円/便KX後は35,814円/便KE後は35,826円/便JI後は37,951円/便JG後は37,934円/便BY修正前は48,377円)', grand, 40051)
    const nabe = starterDefs.find((d) => d.title === '寄せ鍋')
    eq(
      '寄せ鍋 1食あたり(便EY後226円→便FAのしいたけ名寄せで217円→便LFの生しいたけ調べ直しで247円)',
    ...
    eq('中華風卵スープ 1食あたり(便LF後44円。便KE後42円・便JI後82円・修正前682円はごま油「少々」に1Lボトル満額)', Math.round(estimateRecipeCost(soup.ingredients, idx).total / soup.servings), 44)
...
// (オーナー指示。一般的な表記で、成分表・公開ページの食品名とも揃う)。値段と単位は変えていない。
{
  ...
  eq('FA 生しいたけは1項目で280円/6枚(便FAで名寄せ・2026-08-26 便LFで実勢に合わせた)', {
  ...
  // 名寄せ: 表記が違っても同じ1件に解決する / 生と乾燥は別々の1件に解決する
  {
    ...
    eq(
      'FA 素の「しいたけ4枚」も生しいたけの行から出る(280円/6枚で187円。便LFの前は67円)',
      estimateIngredientYen({ name: 'しいたけ', amount: '4', unit: '枚' }, idx)?.yen,
      187,
    )
    eq(
      'FA 生しいたけ2枚は93円(便EYの按分の仕方は変えていない。便LFの前は33円)',
      estimateIngredientYen({ name: '生しいたけ', amount: '2', unit: '枚' }, idx)?.yen,
      93,
    )
    eq(
  ...
  // 既存端末の重複行を1行に畳む移行(規約F: 何が変わって何が残るか)
  {
    ...
    eq(
      ...
      [{ kind: 'rename', id: 1, name: 'しいたけ', toName: '生しいたけ', pricePerUnit: 280, unit: '6枚' }],
    )
    eq(
...
// 「実際はこれより高くなります」という案内が事実と逆になっていた。
{
  ...
  eq('KE-2 ごま油「適量」は9円（大さじ1相当。ボトル1本ぶんが乗らない。便LFの前は6円）', keYen('ごま油', '適量', ''), 9)
  ...
  eq('KE-3 つくねの照り焼きの1食は194円（便LFの前は193円・便KEの直後は186円・直す前は386円）', kePerServing(tsukune, 2), 194)
...
//   ・鶏むね肉 90円/100g → 実勢の中央値 106円/100g（差は+18%で、ほかの4件より小さい）
{
  ...
  for (const [title, before, after] of [
    ['ひじきの煮物', 50, 200], // 第3弾の油揚げ・第5弾のごま油でさらに上がった
    ['きゅうりとわかめの酢の物', 58, 122],
    ['切り干し大根のハリハリ漬け', 67, 166],
    ['しらたきのチャプチェ風', 194, 256], // 第5弾のごま油でさらに上がった
    ['だしのとり方', 70, 144], // 第2弾のかつお節 15→25円/1袋 でさらに上がった
    ['豆腐とわかめの味噌汁', 28, 53],
    ['キャベツの塩昆布あえ', 83, 107], // 第5弾のごま油でさらに上がった
    ['高野豆腐の含め煮', 72, 93],
    ['豆腐グラタン', 158, 181],
    ['鶏ささみの梅しそレンジ蒸し', 146, 201], // 第4弾の青じそ 100→125円/10枚 でさらに上がった
    ['ささみとブロッコリーのごま和え', 134, 194], // 第4弾のブロッコリー 200→280円/1株 でさらに上がった
  ]) {
  ...
  eq('LF-4 「寄せ鍋」は第1弾では動かず、第4弾の生しいたけで217→247円', lfPerServing('寄せ鍋'), 247)
...
//   ±20%は、司令部が渡した「1回目に20%以上ずれていると出た品」の表と同じ線）。
{
  ...
  eq('LF-8 牛薄切り肉は200円/100gのまま（実勢430円・「牛こま切れ肉」がピン留め）', lf2Entry('牛薄切り肉'), [200, '100g'])
  eq('LF-8 牛切り落とし肉も同じ理由で200円/100gのまま', lf2Entry('牛切り落とし肉'), [200, '100g'])
  eq('LF-8 ピン留めされている側も1円も動いていない', [lf2Entry('豚バラ肉'), lf2Entry('牛こま切れ肉')], [[150, '100g'], [200, '100g']])
  ...
  eq('LF-8 ハムは150円/4枚のまま（実勢123円＝-18%）', lf2Entry('ハム'), [150, '4枚'])
  ...
  // --- LF-9: 実際の分量で按分できる ---
  eq('LF-9 豚ひき肉150gは270円', lf2Yen('豚ひき肉', '150', 'g'), 270)
  eq('LF-9 鶏ひき肉200gは300円', lf2Yen('鶏ひき肉', '200', 'g'), 300)
  eq('LF-9 ウインナー4本は140円', lf2Yen('ウインナー', '4', '本'), 140)
  eq('LF-9 春雨50gは100円', lf2Yen('春雨', '50', 'g'), 100)
  eq('LF-9 パン粉1/2カップ(20g)は22円', lf2Yen('パン粉', '1/2', 'カップ'), 22)
...
// ---------- 便LF 第3弾（2026-08-26）: 豆腐・大豆加工品・乳製品と、便LDから回ってきたチーズ ----------
{
  ...
  eq('LF-11 粉チーズは620円/80gのまま（実勢570円＝-8%）', lf3Entry('粉チーズ'), [620, '80g'])
  ...
  // 分量で按分できる
  eq('LF-12 スライスチーズ2枚(36g)は74円', lf3Yen('スライスチーズ', '2', '枚'), 74)
  eq('LF-12 クリームチーズ100gは285円', lf3Yen('クリームチーズ', '100', 'g'), 285)
...
// 野菜は季節で動くので、**調べた日（2026-08-26）を各行のコメントに必ず書いてある**。
{
  ...
  eq('LF-16 にんにく1かけは25円', lf4Yen('にんにく', '1', 'かけ'), 25)
  eq('LF-16 にんにく1片も同じ25円（書き方ちがい）', lf4Yen('にんにく', '1', '片'), 25)
  eq('LF-16 生しいたけ4枚は187円', lf4Yen('生しいたけ', '4', '枚'), 187)
  eq('LF-16 ブロッコリー1/2株は140円', lf4Yen('ブロッコリー', '1/2', '株'), 140)
  eq('LF-16 青じそ2枚は25円', lf4Yen('青じそ', '2', '枚'), 25)
  eq('LF-16 大葉2枚も同じ25円', lf4Yen('大葉', '2', '枚'), 25)
}
...
// 1Lあたりに直してから比べた。マスタの単位「1L」は変えていない。
{
  ...
  // --- LF-17: 直した行 ---
  eq('LF-17 ごま油の目安価格が1,700円/1L', lf5Entry('ごま油'), [1700, '1L'])
  eq('LF-17 オリーブオイルの目安価格が2,100円/1L', lf5Entry('オリーブオイル'), [2100, '1L'])
  eq('LF-17 米油の目安価格が880円/1L', lf5Entry('米油'), [880, '1L'])
  ...
  eq('LF-18 酢は340円/1Lのまま（何を「酢」とするかで上にも下にも動く）', lf5Entry('酢'), [340, '1L'])
  ...
  eq('LF-19 ごま油「少々」は小さじ1相当の9円', lf5Yen('ごま油', '少々', ''), 9)
  eq('LF-19 ごま油「適量」も同じ9円', lf5Yen('ごま油', '適量', ''), 9)
  eq('LF-19 オリーブオイル「適量」は大さじ1相当の32円', lf5Yen('オリーブオイル', '適量', ''), 32)
  eq('LF-19 ごま油大さじ1は26円', lf5Yen('ごま油', '1', '大さじ'), 26)
  eq('LF-19 オリーブオイル大さじ1は32円', lf5Yen('オリーブオイル', '1', '大さじ'), 32)
}
