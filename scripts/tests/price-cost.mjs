...
// 原価が過大(サラダ油「適量」=1Lボトル400円)・過小(鶏むね肉「1枚」=100g分90円)の両方向に壊れていた ----------
{
  ...
  // (8) 同梱109品の合計原価のピン留め(この数字が動いたら按分の前提が変わったということ)
  {
    ...
    eq('同梱109品の概算食費の合計(便LF第4弾後。便LF第3弾後は38,121円/便LF第2弾後は37,802円/便LF第1弾後は37,015円/便KX後は35,814円/便KE後は35,826円/便JI後は37,951円/便JG後は37,934円/便BY修正前は48,377円)', grand, 39777)
...
// 「実際はこれより高くなります」という案内が事実と逆になっていた。
{
  ...
  // --- KE-3: 実データの品ぜんぶを、1食あたりの金額で見張る ---
  {
    ...
    eq('KE-3 つくねの照り焼きの1食は193円（便KEの直後は186円・直す前は386円）', kePerServing(tsukune, 2), 193)
...
//   ・鶏むね肉 90円/100g → 実勢の中央値 106円/100g（差は+18%で、ほかの4件より小さい）
{
  ...
  for (const [title, before, after] of [
    ['ひじきの煮物', 50, 198], // 第3弾の油揚げ 20→30円/1枚 でさらに上がった
    ['きゅうりとわかめの酢の物', 58, 122],
  ...
  'カットわかめ', 'ごま油', 'オリーブオイル', '揚げ油', '酒', 'みりん', '酢',
