...
// 原価が過大(サラダ油「適量」=1Lボトル400円)・過小(鶏むね肉「1枚」=100g分90円)の両方向に壊れていた ----------
{
  ...
  // (8) 同梱109品の合計原価のピン留め(この数字が動いたら按分の前提が変わったということ)
  {
    ...
    // にんにく 60→107円/1玉・牛こま切れ肉/牛薄切り肉/牛切り落とし肉 200→253円/100g（3行そろえて）。
    // 2026-08-26 便LF 第9弾（オーナー裁定「ORIGINAL_30 のピン留めを外して並の実勢へ」）で
    // 38,932→45,450円(+6,518円)。**司令部が出した確かめの値44,568円との差6,518-5,636=882円は、
    // 豚ロース薄切り 180→278円/100g のぶん**（司令部の指示にあったが、確かめの値には入っていなかった）。
    // 上がった19件のほかに**キャベツは130→75円に下がる**（-264円）。
    // 2026-08-26 便LF 第10弾（残りの主食・魚・ひき肉）で45,450→47,430円(+1,980円)。
    // 鮭・生鮭 120→280円/1切れ・塩鮭 150→230円/1切れ・米 60→130円/1合・合いびき肉 130→210円/100g。
    // 2026-08-26 便LF 第11弾（調味料。司令部裁定「調味料の並は全国銘柄」）で47,430→47,212円(-218円)。
    // しょうゆ 400→278円/1L（54品で使う・-122円/1L）・ケチャップ 960→570・中濃ソース 780→580・
    // ポン酢 890→620 が下がり、マヨネーズ 680→840・カレールー 200→285 が上がって差し引き-218円。
    // 2026-08-26 便LF 第12弾（司令部裁定: みそは据え置き・白練りごまは動かす）で47,212→47,252円(+40円)。
    // 白練りごま 40→60円/大さじ1 のみ。みそ類6件・ごま類6件は**測ったうえで据え置き**（差が±20%未満）。
    // 2026-08-26 便LF 第13弾（缶詰）で47,252→47,662円(+410円)。
    // カットトマト缶 100→160円/1缶・サバ水煮缶 150→310円/1缶。
    // サバみそ煮缶(+19%)・スパゲッティ(単一ソース)・コーン缶(2店で缶の大きさがそろわない)は据え置き。直したのは3件
    ...
    // 2026-08-27 便LL 第1弾（香辛料・薬味・中華調味料の18件）で47,662→47,956円(+294円)。
    // 直したのは6件（こしょう 10→17円/小さじ1・乾燥ハーブ 20→70円/小さじ1・甜麺醤 20→57円/大さじ1・
    // 豆板醤 15→10円/小さじ1・コチュジャン 10→15円/小さじ1・鶏がらスープの素 9→12円/小さじ1）で、動いたのは16品。
    // いちばん大きいのは甜麺醤（回鍋肉 325→351円・豚肉の甜麺醤炒め 373→401円）と
    ...
    // 2026-08-27 便LL 第3弾（生鮮・薬味・主食・卵の14件＋巻き込みで直した2件）で47,838→47,864円(+26円)。
    // 直したのは7件（キウイ 100→170円/1個・食パン 30→37円/1枚・そうめん 50→62円/1束・
    // 錦糸卵 30→37円/1個分・冷凍うどん 100→71円/1玉・卵白 17→25円/1個分・卵黄 8→12円/1個分）と、
    // 巻き込みで1件（黒こしょう 10→16円/小さじ1。こしょうを17円にしたので同じ胡椒がそろわなくなるのを直した）。
    // 動いたのは7品で、**冷凍うどんが下がったぶん（肉うどん 353→324円・梅おろしぶっかけうどん 212→183円）**と
    // 上がったぶんがほぼ打ち消し合っている。
    // 卵黄・卵白は**分け方を変えていない**——便LF 第9弾で卵が25→37円/1個になったのに2行だけ25円のまま
    // 取り残されており、「卵黄＋卵白＝卵1個」の一致が崩れていたのを戻しただけ
    eq('同梱109品の概算食費の合計(便LL第3弾後。便LL第2弾後は47,838円/便LL第1弾後は47,956円/便LF第13弾後は47,662円/便LF第12弾後は47,252円/便LF第11弾後は47,212円/便LF第10弾後は47,430円/便LF第9弾後は45,450円/便LF第8弾後は38,932円/便LF第7弾後は38,147円/便LF第6弾後は39,124円/便LF第5弾後は40,051円/便LF第4弾後は39,777円/便LF第3弾後は38,121円/便LF第2弾後は37,802円/便LF第1弾後は37,015円/便KX後は35,814円/便KE後は35,826円/便JI後は37,951円/便JG後は37,934円/便BY修正前は48,377円)', grand, 47864)
    ...
    // 2026-08-27 便LL: 鶏がらスープの素 9→12円/小さじ1（味の素「丸鶏がらスープ」110g ヤオコー429.84円/
    // 東急ストア462円・並のグレード・調べた日 2026-08-27）で55→58円
    eq('中華風卵スープ 1食あたり(便LL後58円。便LF後55円・便KE後42円・便JI後82円・修正前682円はごま油「少々」に1Lボトル満額)', Math.round(estimateRecipeCost(soup.ingredients, idx).total / soup.servings), 58)
    const steamed = starterDefs.find((d) => d.title.includes('レンジ蒸し鶏'))
    eq('レンジ蒸し鶏 1食あたり(便LFのピン留め解除で115→148円・修正前48円は鶏むね肉1枚が100g分の90円だった)', Math.round(estimateRecipeCost(steamed.ingredients, idx).total / steamed.servings), 148)
    const teriyaki = starterDefs.find((d) => d.title === '鶏の照り焼き')
    eq('鶏の照り焼き 1食あたり(便LFの調べ直しで343→429円・修正前280円は鶏もも肉2枚が100g分の130円だった)', Math.round(estimateRecipeCost(teriyaki.ingredients, idx).total / teriyaki.servings), 429)
  }
}
...
// 直す前はすべて赤になることを確認してから直している。
{
  ...
  // 2026-08-27 便LL: 卵が25→37円/1個（2026-08-26 便LF 第9弾のピン留め解除）になったのに卵黄・卵白だけ
  // 25円のまま取り残されており、**この節が見張っている「合計＝卵1個の値段」が崩れていた**。
  // 分け方（卵黄1/3・卵白2/3）は1文字も変えず、元にする卵の値段だけ今の目安価格に合わせた
  // （卵黄12円・卵白25円）。**卵の値段を書き写さず、マスタから引いて突き合わせる**形にしたので、
  // 次に卵が動いてもこの節は「一致しているか」だけを見る（禁じ手③を避ける）
  const jgEgg = () => PRICE_DEFAULTS.find((d) => d.name === '卵').pricePerUnit
  eq('JG-3 卵黄2個分は12円×2', jgYen('卵黄', '2', '個分'), 24)
  eq('JG-3 卵白2個分は25円×2', jgYen('卵白', '2', '個分'), 50)
  eq('JG-3 卵黄1個分＋卵白1個分＝卵1個の値段', jgYen('卵黄', '1', '個分') + jgYen('卵白', '1', '個分'), jgEgg())
  ...
  // 2026-08-27 便LL: こしょうを10→17円/小さじ1に直したので2→3円になった（0.3g÷小さじ1=2g×17円＝2.55円）。
  // 見張っているのは「小さじ1杯まるごと(いまなら17円)が1行に乗らないこと」なので、その意味は変わっていない。
  // 実測の帯: ヤオコー エスビー食品「袋入り コショー」14g 106.92円＝7.64円/g ／
  //   東急ストア エスビー食品「テーブルコショー」50g 473円＝9.46円/g（調べた日 2026-08-27・税込・並のグレード）
  eq('JG-4 分量なしのこしょうは約3円（0.3g÷小さじ1=2g×17円）', jgYen('こしょう', '', ''), 3)
  eq('JG-4 「少々」と書いたこしょうも同じ', jgYen('こしょう', '少々', ''), 3)
  eq('JG-4 黒こしょうも同じ', jgYen('黒こしょう', '少々', ''), 2)
  // 分量が数値で書いてあるときは今までどおり按分する（仮の量に置き換えない）
  // 2026-08-27 便LL: 17円/小さじ1 の半分で9円（10円だったときは5円）
  eq('JG-4 こしょう小さじ1/2は登録単位の半分', jgYen('こしょう', '1/2', '小さじ'), 9)
  ...
  // 原価の行は「約6円」（100円÷17人分）で、半分の卵の値段（約12円）と合っていなかった
  {
    ...
    // 2026-08-26 便LF: 卵を25→37円/1個（並の実勢）にしたので、金額はどれも1.48倍になった。
    // **この節が見ているのは「画面に出ている人数分に金額が追随すること」**で、値そのものではない
    eq('JG-5 登録人数のままなら全量ぶん(卵4個=148円。便LFの前は100円)', row17?.shownYen, 148)
    eq('JG-5 2人分にすると2/17ぶん(約17円)になる', row2?.shownYen, 17)
    eq('JG-5 人数を増やすと金額も増える', row4?.shownYen, 35)
    eq('JG-5 1食あたりの値は今までどおり登録人数で割った値', row2?.perServingYen, 9)
  }
  ...
  for (const [name, pricePerUnit, unit] of [
    // 2026-08-27 便LL: 卵が25→37円/1個になったのに取り残されていたので、同じ分け方のまま
    // 卵黄12円（1/3）・卵白25円（2/3）へ直した（根拠は priceDefaults.ts の行）
    ['卵黄', 12, '1個分'],
    ['卵白', 25, '1個分'],
    ...
    ['粉砂糖', 15, '大さじ1'],
    ['青ねぎ', 80, '100g'],
    ['塩鮭', 230, '1切れ'], // 2026-08-26 便LF: 並のグレードで150→230円（根拠は priceDefaults.ts の行）
    // 2026-08-27 便LL: 10→16円。「こしょう」を17円に直したので、同じ胡椒が書き方で1.7倍ちがわないようそろえた
    // （ハウス食品「ギャバン ブラックペッパー 缶」70g ヤオコー548.64円／東急ストア592円・並のグレード・調べた日 2026-08-27）
    ['黒こしょう', 16, '小さじ1'], // 項目名は「粗びき黒こしょう」からの改名
  ]) {
...
//   ⑤実勢と桁で食い違っていた目安価格が直っている
{
  ...
  // --- JI-2: 古い目安のままの行だけを新しい目安価格に入れ替える ---
  {
    const rows = [
      ...
      // 2026-08-26 便LF: 玉ねぎは並の実勢77円/1個へ動かしたので「既に今の目安と同じ」の例に使えなくなった。
      // いまも投入時の値のままの「みそ 11円/大さじ1」に差し替える（この節が見ているのは
      // 「今の目安と同じ行は触らない」ことで、どの食材かではない）
      jiRow(3, 'みそ', 11, '大さじ1'), // 既に今の目安と同じ → 触らない
    ]
...
// 「実際はこれより高くなります」という案内が事実と逆になっていた。
{
  ...
  // 2026-08-26 便LF: しょうゆを400→278円/1L（全国銘柄の並）にしたので6→4円。
  // 便KEが見張っているのは「『大匙1』と書いても1L1本ぶんの金額が乗らないこと」で、値そのものではない
  eq('KE-1 醤油「大匙1」は4円（旧: 1L1本ぶんの400円。便LFの前は6円）', keYen('醤油', '大匙1', ''), 4)
  ...
  // 2026-08-27 便LL: こしょう10→17円/小さじ1 で2→3円（按分の仕組みは変えていない。根拠は priceDefaults.ts の行）
  eq('KE-2 分量なしのこしょうは3円（便JG-4の按分のまま）', keYen('こしょう', '', ''), 3)
  ...
  eq('KE-3 つくねの照り焼きの1食は237円（便LFの調べ直しで194→237円・便KEの直後は186円・直す前は386円）', kePerServing(tsukune, 2), 237)
...
// 新しい重さの数値は1つも作っていない（根拠は scripts/nutrition-foods.mjs の各noteに1件ずつ）。
{
  ...
  // 2026-08-26 便LF: しょうゆの調べ直しで75→74円
  eq('KL-3 煮浸しは1食74円（旧8円＝実勢の約1/9）', klPerServingYen(klNibitashi, 4), 74)
  eq(
...
//   ・鶏むね肉 90円/100g → 実勢の中央値 106円/100g（差は+18%で、ほかの4件より小さい）
{
  ...
  // 2026-08-26 オーナー裁定で ORIGINAL_30 のピン留めを外し、並の実勢へ動かした（LF-22が見ている）
  ...
  for (const [title, before, after] of [
    ['ひじきの煮物', 50, 113], // 第6弾でひじきを判断待ちに戻したので、油揚げのぶんだけ残った
    ['きゅうりとわかめの酢の物', 58, 110], // 第6弾でわかめを判断待ちに戻したので元に戻った
    ['切り干し大根のハリハリ漬け', 67, 69], // 第6弾で判断待ちに戻したので元に戻った
    ['しらたきのチャプチェ風', 194, 252], // 第5弾のごま油でさらに上がった
    ['だしのとり方', 70, 139], // 第2弾のかつお節 15→25円/1袋 でさらに上がった
    ['豆腐とわかめの味噌汁', 28, 45], // 第6弾でわかめを判断待ちに戻したので元に戻った
    ['キャベツの塩昆布あえ', 83, 67], // 第5弾のごま油でさらに上がった
    ['高野豆腐の含め煮', 72, 100],
    ['豆腐グラタン', 158, 260],
    ['鶏ささみの梅しそレンジ蒸し', 146, 202], // 第4弾の青じそ 100→125円/10枚 でさらに上がった
  ...
  eq('LF-4 「寄せ鍋」は第9弾のピン留め解除で241→315円', lfPerServing('寄せ鍋'), 315)
  ...
  // （増えても減っても赤になるので、直したら一緒にここも直す）。
  {
    ...
    // 2026-08-27 便LL 第3弾（生鮮・薬味・主食・卵の14件）で 14→0件。**便LFの残りはこれで全部片づいた。**
...
//   ±20%は、司令部が渡した「1回目に20%以上ずれていると出た品」の表と同じ線）。
{
  ...
  // 豚バラ肉・豚バラ薄切り・豚ロース薄切りも同じ裁定で並の実勢へ動かした（LF-22が見ている）
...
// ---------- 便LF 第3弾（2026-08-26）: 豆腐・大豆加工品・乳製品と、便LDから回ってきたチーズ ----------
{
  ...
  // 2026-08-26 オーナー裁定で豆腐の3行は並の実勢85円/1丁へそろえて動かした（LF-22が見ている）
...
// 野菜は季節で動くので、**調べた日（2026-08-26）を各行のコメントに必ず書いてある**。
{
  ...
  // ⑤ピン留めされた同じ食材の行があるもの
  // ねぎと長ねぎは 2026-08-26 のオーナー裁定で並の実勢139円/1本へそろえて動かした（LF-22が見ている）
...
// ③が書けない＝グレードを決めずに採ったということなので、そこで止まる。
{
  ...
  // 単位は「kg」も見る（米は5kg入りで売っている。2026-08-26 便LF 第10弾で足した）
  const LF6_PRICE_WITH_SIZE = /\d+(?:\.\d+)?\s*(?:kg|g|ml|L|枚|本|個|袋|パック|コ|株|束|玉|切|かけ|合|人前)[^\n]{0,40}?[\d,]+(?:\.\d+)?\s*円/
  ...
  // 2026-08-26 便LF: 逃がすのは「**自分では店を1つも挙げず**、別の行に根拠を預けている行」だけ。
  // 「みそ」のように、自分の行で店と商品を挙げたうえで線の理由だけ別の行（しょうゆ）を指す書き方も
  // あるので、「◯◯の行のコメント」が出てくるだけで逃がすと、根拠を持っている行まで見なくなる
  const lf6IsRef = (b) => lf6RefOf(b.comment) != null && !LF6_SHOP.test(b.comment)
  const lf6Refs = lf6All.filter(lf6IsRef)
  const lf6Mine = lf6All.filter((b) => !lf6IsRef(b))
  eq(
...
// ---------- 便LF 第9弾（2026-08-26）: ORIGINAL_30 のピン留めを外して並の実勢へ ----------
// 【2026-08-26 オーナー裁定】「ピン留めを外して、便LFが測った並の実勢に合わせる」。
// 司令部の言葉「上がるものだけ動かすと、それは実勢に合わせたことにならない。**下がるものも同じように**」。
// **ピン留めの仕組みそのものは残す**（scripts/test-price.mjs の ORIGINAL_30。値が勝手に動かない見張り）。
// 1件ずつの店名・商品名・容量・税込価格・調べた日は src/data/priceDefaults.ts の各行のコメント。
{
  const lf9 = (name) => {
    const e = PRICE_DEFAULTS.find((d) => d.name === name)
    return e ? [e.pricePerUnit, e.unit] : null
  }
  const lf9Index = buildPriceIndex(PRICE_DEFAULTS.map((d) => ({ ...d, isDefault: true })))
  const lf9Yen = (name, amount, unit) => estimateIngredientYen({ name, amount, unit }, lf9Index)?.yen ?? null

  // --- LF-22: 並の実勢へ動かした20件 ---
  for (const [name, pricePerUnit, unit, before] of [
    ['玉ねぎ', 77, '1個', 50],
    ['にんじん', 91, '1本', 40],
    ['じゃがいも', 77, '1個', 40],
    ['キャベツ', 75, '1/4個', 130], // ここだけ下がる
    ['白菜', 214, '1/4個', 150],
    ['大根', 161, '1/2本', 100],
    ['もやし', 37, '1袋', 30],
    ['きゅうり', 80, '1本', 40],
    ...
    eq(`LF-22 「${name}」が${before}→${pricePerUnit}円/${unit}`, lf9(name), [pricePerUnit, unit])
  }
  // **下がるものも動かす**（キャベツ）。ここが130円に戻っていたら「上がるものだけ動かした」ことになる
  eq('LF-22 キャベツは下がる（130→75円/1-4個）', lf9('キャベツ')[0] < 130, true)

  // --- LF-23: 同じ食材が別名で並ぶ組は、まとめて動かす（1行だけ動かすと書き方で値段が変わる） ---
  eq('LF-23 豆腐・木綿豆腐・絹ごし豆腐が同じ値', [lf9('木綿豆腐'), lf9('絹ごし豆腐')], [lf9('豆腐'), lf9('豆腐')])
  eq('LF-23 豚バラ肉・豚バラ薄切りが同じ値', lf9('豚バラ薄切り'), lf9('豚バラ肉'))
  eq('LF-23 牛こま切れ肉・牛薄切り肉・牛切り落とし肉が同じ値', [lf9('牛薄切り肉'), lf9('牛切り落とし肉')], [lf9('牛こま切れ肉'), lf9('牛こま切れ肉')])
  eq('LF-23 ねぎ・長ねぎが同じ値', lf9('長ねぎ'), lf9('ねぎ'))
  // 豚ロース薄切りも一緒に動かす。据え置くと「ロースがバラより安い」逆転が表に残る
  // （実勢はバラ300円・ロース278円でバラのほうが高い）
  eq('LF-23 豚ロース薄切りは並の278円/100g', lf9('豚ロース薄切り'), [278, '100g'])
  eq('LF-23 豚バラのほうが豚ロースより高い（実勢と同じ並び）', lf9('豚バラ肉')[0] > lf9('豚ロース薄切り')[0], true)

  // --- LF-24: まだ並のグレードで測っていないピン留めは、据え置いたうえで行にそう書いてある ---
  // 次に測る便がここから拾えるようにする。ピン留めが外れた＝勝手に動かしてよい、ではない
  for (const [name, pricePerUnit, unit] of [
    ['豚こま切れ肉', 110, '100g'],
    ['さば', 100, '1切れ'], // 並のさば切り身が2店でそろわない
    ['ピーマン', 30, '1個'], // 測ったが単一ソース
    ['バター', 600, '200g'], // 測った結果 588円＝-2%
    ['みそ', 11, '大さじ1'], // 2026-08-25 司令部裁定で据え置き
  ]) {
    eq(`LF-24 「${name}」は${pricePerUnit}円/${unit}のまま`, lf9(name), [pricePerUnit, unit])
  }

  // --- LF-25: 実際の分量で按分できる（単位は1つも変えていない） ---
  eq('LF-25 玉ねぎ1個は77円（e2e PRICE-01 が見ているのと同じ値）', lf9Yen('玉ねぎ', '1', '個'), 77)
  eq('LF-25 玉ねぎ1/2個は39円', lf9Yen('玉ねぎ', '1/2', '個'), 39)
  eq('LF-25 鶏もも肉300gは495円', lf9Yen('鶏もも肉', '300', 'g'), 495)
  eq('LF-25 豆腐1/2丁は43円', lf9Yen('豆腐', '1/2', '丁'), 43)
  eq('LF-25 木綿豆腐1/2丁も同じ43円（書き方で値段が変わらない）', lf9Yen('木綿豆腐', '1/2', '丁'), 43)
  eq('LF-25 卵2個は74円', lf9Yen('卵', '2', '個'), 74)
  eq('LF-25 キャベツ1/4個は75円（下がったぶんも按分に効く）', lf9Yen('キャベツ', '1/4', '個'), 75)
}

// ---------- 便LF 第11弾（2026-08-26）: 調味料。**並＝全国銘柄**（司令部裁定） ----------
// 【線の決め方】選ぶのは「安いほう」ではなく「並＝普通に買われているもの」。
// わかめ・ひじきで輸入を採ったのは安いからではなく**輸入が家庭の定番で、国産がその上のグレード**だから。
// 調味料はそこが逆で、**全国銘柄（キッコーマン・カゴメ・キユーピー等）が家庭の定番、PBがその下のライン**。
// ここでPBを採ると、同じ「並」という言葉が品によって違うグレードを指すことになる。
// **PBの値も行に残す**（産地と同じ形で、あとから振り直せるように）。
{
  const lf11 = (name) => {
    const e = PRICE_DEFAULTS.find((d) => d.name === name)
    return e ? [e.pricePerUnit, e.unit] : null
  }
  const lf11Index = buildPriceIndex(PRICE_DEFAULTS.map((d) => ({ ...d, isDefault: true })))
  const lf11Yen = (name, amount, unit) => estimateIngredientYen({ name, amount, unit }, lf11Index)?.yen ?? null
  const lf11Src = readFileSync(path.join(path.dirname(fileURLToPath(scriptFileUrl)), '..', 'src/data/priceDefaults.ts'), 'utf-8')

  // --- LF-26: 直した6件（同じ銘柄・同じ容量で2店をそろえてある） ---
  for (const [name, pricePerUnit, unit, before] of [
    ['しょうゆ', 278, '1L', 400], // 同梱109品の54品で使う＝効きがいちばん大きい
    ['ケチャップ', 570, '1L', 960],
    ['マヨネーズ', 840, '1L', 680],
    ['中濃ソース', 580, '1L', 780],
    ['ポン酢', 620, '1L', 890],
    ...
    eq(`LF-26 「${name}」が${before}→${pricePerUnit}円/${unit}`, lf11(name), [pricePerUnit, unit])
  }
  // **なぜPBではなく全国銘柄を採ったか**が行に書いてあること。
  // ここが無いと、次の便が「他はPBなのに、ここだけ違う」と見て戻してしまう
  eq('LF-26 しょうゆの行に「調味料の並は全国銘柄」の理由が書いてある', /調味料の並は全国銘柄/.test(lf11Src), true)
  eq('LF-26 しょうゆの行にPBの並160円も残してある', /PBの並[\s\S]{0,200}160円\/1L/.test(lf11Src), true)

  // --- LF-27: 測ったうえで動かさなかったもの ---
  eq('LF-27 減塩しょうゆは570円/1Lのまま（実勢633円＝+11%）', lf11('減塩しょうゆ'), [570, '1L'])
  eq('LF-27 めんつゆは420円/1Lのまま（実勢471円＝+12%）', lf11('めんつゆ'), [420, '1L'])
  eq('LF-27 白だしは630円/1Lのまま（実勢536円＝-15%）', lf11('白だし'), [630, '1L'])
  // 2店で銘柄も箱の大きさもそろわず、比べられなかった
  eq('LF-27 シチュールーは250円/1箱のまま（2店で同じ規格がそろわない）', lf11('シチュールー'), [250, '1箱'])

  // --- LF-28: 実際の分量で按分できる（単位は1つも変えていない） ---
  eq('LF-28 しょうゆ大さじ1は4円（便LFの前は6円）', lf11Yen('しょうゆ', '1', '大さじ'), 4)
  eq('LF-28 しょうゆ100mlは28円', lf11Yen('しょうゆ', '100', 'ml'), 28)
  eq('LF-28 マヨネーズ大さじ1は13円', lf11Yen('マヨネーズ', '1', '大さじ'), 13)
  eq('LF-28 ケチャップ大さじ1は9円', lf11Yen('ケチャップ', '1', '大さじ'), 9)
  eq('LF-28 中濃ソース大さじ1は9円', lf11Yen('中濃ソース', '1', '大さじ'), 9)
  eq('LF-28 ポン酢大さじ1は9円', lf11Yen('ポン酢', '1', '大さじ'), 9)
  // 「適量」「少々」に1本ぶんの金額が乗らないこと（便BY・便KEの直しを壊していない）
  eq('LF-28 しょうゆ「適量」は1回に使う量のまま', lf11Yen('しょうゆ', '適量', '') < 20, true)
}

// ---------- 便LF 第12弾（2026-08-26）: 司令部裁定（みそは据え置き・白練りごまは動かす） ----------
// 【司令部から足された線】「±20%に届かないものは動かさない」は良い線だが、
// **測ったこと自体は残す**——「測って、差が小さかったので動かさなかった」と「まだ測っていない」は
// 別のことで、行を見て区別できる必要がある。ここではそれを機械で見る。
{
  const lf12 = (name) => {
    const e = PRICE_DEFAULTS.find((d) => d.name === name)
    return e ? [e.pricePerUnit, e.unit] : null
  }
  const lf12Src = readFileSync(path.join(path.dirname(fileURLToPath(scriptFileUrl)), '..', 'src/data/priceDefaults.ts'), 'utf-8')
  const lf12Body = lf12Src.slice(lf12Src.indexOf('export const PRICE_DEFAULTS: PriceDefaultItem[] = ['))
  const lf12Comment = (name) => {
    const buf = []
    let cur = []
    for (const raw of lf12Body.split('\n')) {
      const line = raw.trim()
      const item = line.match(/^\{ name: '(.+?)', pricePerUnit: \d+, unit: '.+?' \},$/)
      if (item) {
        if (item[1] === name) return cur.join('\n')
        cur = []
        continue
      }
  ...
  // --- LF-29: 白練りごまは動かす（2店で同じグレードがそろい、差が+50%） ---
  eq('LF-29 白練りごまが40→60円/大さじ1', lf12('白練りごま'), [60, '大さじ1'])

  // --- LF-30: みそ類は据え置き（並は全国銘柄。PBの6円は採らない） ---
  for (const [name, price] of [['みそ', 11], ['白みそ', 13], ['赤みそ', 11], ['だし入りみそ', 11], ['麦みそ', 11], ['豆みそ', 30], ['減塩みそ', 11]]) {
    eq(`LF-30 「${name}」は${price}円/大さじ1のまま`, lf12(name), [price, '大さじ1'])
  }
  // **次の便が同じ調査をやり直さないように、測った両方の値と、どちらを採ったかが行に書いてある**
  const lf12Miso = lf12Comment('みそ')
  eq('LF-30 みその行にPBの並6円/大さじ1が書いてある', /PBの並[\s\S]{0,300}6円\/大さじ1/.test(lf12Miso), true)
  eq('LF-30 みその行に全国銘柄の並9.9円/大さじ1が書いてある', /全国銘柄の並[\s\S]{0,200}9\.9円\/大さじ1/.test(lf12Miso), true)
  eq('LF-30 みその行に「採ったのは全国銘柄」と書いてある', /採ったのは全国銘柄/.test(lf12Miso), true)
  eq('LF-30 みその行に据え置きの理由（差が-10%）が書いてある', /-10%/.test(lf12Miso), true)

  // --- LF-31: ごま類は測ったうえで据え置き（いりごま+12%・すりごま-15%） ---
  for (const name of ['白いりごま', '白ごま', '黒いりごま', 'いりごま', '白すりごま', 'すりごま']) {
    eq(`LF-31 「${name}」は15円/大さじ1のまま`, lf12(name), [15, '大さじ1'])
    // 「測って据え置いた」と「まだ測っていない」を行から区別できること
    eq(`LF-31 「${name}」の行に測ったうえで据え置いたと書いてある`, /測ったうえで据え置き/.test(lf12Comment(name)), true)
  }
}

// ---------- 便LF 第13弾（2026-08-26）: 缶詰 ----------
{
  const lf13 = (name) => {
    const e = PRICE_DEFAULTS.find((d) => d.name === name)
    return e ? [e.pricePerUnit, e.unit] : null
  }
  // --- LF-32: 直した2件（2店とも同じ銘柄・同じ缶の大きさでそろえた） ---
  eq('LF-32 カットトマト缶が100→160円/1缶（2店とも400g缶）', lf13('カットトマト缶'), [160, '1缶'])
  eq('LF-32 サバ水煮缶が150→310円/1缶（2店ともUmiosの190g缶）', lf13('サバ水煮缶'), [310, '1缶'])
  // --- LF-33: 測ったうえで据え置き（「まだ測っていない」と区別できるよう、行に理由が書いてある） ---
...
// ---------- 便LL（2026-08-27）: 便LFがやり残した42件に根拠を書く ----------
//
// 【便LLが使った物差し】便LFと同じ（docs/75）。店は2つだけ・各店の**並のグレード**を1つずつ採って平均・
// ±20%に届かない差は動かさない・単一ソースは動かさない・調味料の並は全国銘柄。
//
// 【便LLが新しく決めたこと: 「1回に使う量」で登録されている行の扱い】
// 価格マスタには、販売単位（1L・100g・1個）ではなく**1回の調理で使う量**を単位にした行がある
// （大さじ1・小さじ1・少々・使用分・1個分。logic/priceEstimate.ts の SINGLE_USE_MASTER_UNITS）。
// **店の棚の値段をそのまま入れられない**ので、便LLは次の1本の線で分けた:
//  ①**単位が「大さじ1」「小さじ1」「1本」「1個分」のように量が数で決まっている行**
//    …棚の並のグレードの単価（円/g・円/ml）に、**アプリ自身が持っている換算**
//      （成分表の unitGrams・大さじ15ml/小さじ5ml）を掛けて出す。**新しい係数を作らない。**
//  ②**単位が「少々」「使用分」のように量が数で決まっていない行**
//    …**棚の並のグレードまでは測って行に書き残すが、値は動かさない。**
//      1回に使う量が何gかをアプリが持っていない（logic/amountAssumption.ts の TYPICAL_AMOUNTS に無い）ので、
//      円/g から1回ぶんの金額に直せない。**量をここで新しく決めるのは物差しの外**
//      （「根拠が書けないものは動かさない」）。司令部が量を決めれば、行のコメントの円/g からすぐ出せる。
//
// 【LL-1 の見張り】便LFの LF-20 と同じ形で、**便LLが根拠を書いた行は
// ①店名 ②商品名と容量と税込価格の組 ③どのグレードで採ったか（「並」または「標準品」の語）**が
// そろっていることを機械で見る。2店とも扱いが無くて棚の値段そのものが取れなかった行だけは
// LL_NO_SHELF_PRICE に**理由つきで**並べて逃がす（増やすときは必ず理由を書くこと）。
{
  const llRoot = path.join(path.dirname(fileURLToPath(scriptFileUrl)), '..')
  const llSrc = readFileSync(path.join(llRoot, 'src/data/priceDefaults.ts'), 'utf-8')
  const llBody = llSrc.slice(llSrc.indexOf('export const PRICE_DEFAULTS: PriceDefaultItem[] = ['))
  const llBlocks = []
  let llBuf = []
  for (const rawLine of llBody.split('\n')) {
    const line = rawLine.trim()
    const item = line.match(/^\{ name: '(.+?)', pricePerUnit: (\d+), unit: '(.+?)' \},$/)
    if (item) {
      llBlocks.push({ name: item[1], comment: llBuf.join('\n') })
      llBuf = []
      continue
    }
    if (line.startsWith('//')) llBuf.push(line)
    else if (line !== '') llBuf = []
  }
  const LL_SHOP = /ヤオコー|東急ストア/
  const LL_PRICE_WITH_SIZE = /\d+(?:\.\d+)?\s*(?:kg|g|ml|L|枚|本|個|袋|パック|コ|株|束|玉|切|かけ|合|人前|尾|缶|丁|食)[^\n]{0,40}?[\d,]+(?:\.\d+)?\s*円/
  const LL_GRADE = /並|標準品/
  // 2店とも扱いが無く、棚の値段そのものが取れなかった行（②の免除。理由は各行のコメント）
  const LL_NO_SHELF_PRICE = [
    // 2店とも生の丸のさんまが棚に無い（缶詰と干物だけ）
    'さんま',
    // 2店ともすだちが棚に無い（すだち風味の飲料・つゆだけ）
    'すだち',
  ]
  ...
  // 指し先は便LFが根拠を書いた行でもよい（「卵」のように便LFが測った行を指すことがある）。
  // 見るのは「指し先の行が自分で店を挙げているか」だけ
  eq(
    'LL-1 別の行を指しているコメントは、指し先の行が自分で店を挙げている',
    llRefs
      .filter((b) => {
        const target = llBlocks.find((x) => x.name === llRefOf(b.comment))
        return !target || !LL_SHOP.test(target.comment)
      })
      .map((b) => b.name),
    [],
  )
  ...
  eq(
    'LL-1 ②商品の容量と税込価格の組が書いてある',
    llMissing(LL_PRICE_WITH_SIZE, (b) => !LL_NO_SHELF_PRICE.includes(b.name)),
    [],
  )
  eq('LL-1 ③どのグレードで採ったか（並・標準品）が書いてある', llMissing(LL_GRADE), [])
  eq(
    'LL-1 棚の値段が取れなかった行は、扱いが無いことを行に書いてある',
    LL_NO_SHELF_PRICE.filter((n) => {
      const b = llBlocks.find((x) => x.name === n)
      return !b || !/扱いが無|置いていない|扱っていない/.test(b.comment)
    }),
    [],
  )
