...
// 原価が過大(サラダ油「適量」=1Lボトル400円)・過小(鶏むね肉「1枚」=100g分90円)の両方向に壊れていた ----------
{
  ...
  // (8) 同梱109品の合計原価のピン留め(この数字が動いたら按分の前提が変わったということ)
  {
    ...
    // 上がった19件のほかに**キャベツは130→75円に下がる**（-264円）。直したのは3件
    ...
    eq('同梱109品の概算食費の合計(便LF第9弾後。便LF第8弾後は38,932円/便LF第7弾後は38,147円/便LF第6弾後は39,124円/便LF第5弾後は40,051円/便LF第4弾後は39,777円/便LF第3弾後は38,121円/便LF第2弾後は37,802円/便LF第1弾後は37,015円/便KX後は35,814円/便KE後は35,826円/便JI後は37,951円/便JG後は37,934円/便BY修正前は48,377円)', grand, 45450)
...
//   ・鶏むね肉 90円/100g → 実勢の中央値 106円/100g（差は+18%で、ほかの4件より小さい）
{
  ...
  // （増えても減っても赤になるので、直したら一緒にここも直す）。
  {
    ...
    const LF_NO_SOURCE_KNOWN = [
      '合いびき肉', 'さば', '赤唐辛子', 'しょうが', 'さんま', 'すだち', '刻みねぎ', '生鮭',
      'むきえび', 'サバ水煮缶', '錦糸卵', 'スパゲッティ', '冷凍うどん', '食パン', '揚げ油', 'だし汁',
      '水またはだし汁', '中濃ソース', 'ケチャップ', 'マヨネーズ', 'ポン酢', 'めんつゆ', 'カレールー', 'シチュールー',
      '鶏がらスープの素', 'おろしにんにく', '塩', '塩こしょう', 'こしょう', '七味唐辛子', '砂糖', '甜麺醤',
      '豆板醤', '粉山椒', 'ラー油', '紅しょうが', '刻みのり', '白ごま', '白すりごま', 'すりごま',
      '黒いりごま', 'いりごま', '白練りごま', 'カットトマト缶', 'みかん缶', 'メープルシロップ', '黒みつ', 'アーモンドエッセンス',
      'さわら', '生だら', '万能ねぎ', '梅干し', 'そうめん', 'グラノーラ', 'こしあん', 'キウイ',
      'はちみつ', 'オイスターソース', 'コチュジャン', 'カレー粉', '乾燥ハーブ', '卵白',
    ]
    eq(
