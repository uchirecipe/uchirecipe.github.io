...
// 原価が過大(サラダ油「適量」=1Lボトル400円)・過小(鶏むね肉「1枚」=100g分90円)の両方向に壊れていた ----------
{
  ...
  // (8) 同梱109品の合計原価のピン留め(この数字が動いたら按分の前提が変わったということ)
  {
    ...
    // 下がったのは、棚の商品を全部並べた中央値ではなく**各店の標準品を1つずつ**で採り直したため。直したのは3件
    ...
    eq('同梱109品の概算食費の合計(便LF第7弾後。便LF第6弾後は39,124円/便LF第5弾後は40,051円/便LF第4弾後は39,777円/便LF第3弾後は38,121円/便LF第2弾後は37,802円/便LF第1弾後は37,015円/便KX後は35,814円/便KE後は35,826円/便JI後は37,951円/便JG後は37,934円/便BY修正前は48,377円)', grand, 38147)
...
//   ・鶏むね肉 90円/100g → 実勢の中央値 106円/100g（差は+18%で、ほかの4件より小さい）
{
  ...
  for (const [title, before, after] of [
    ['ひじきの煮物', 50, 53], // 第6弾でひじきを判断待ちに戻したので、油揚げのぶんだけ残った
    ['きゅうりとわかめの酢の物', 58, 57], // 第6弾でわかめを判断待ちに戻したので元に戻った
    ['切り干し大根のハリハリ漬け', 67, 65], // 第6弾で判断待ちに戻したので元に戻った
    ['しらたきのチャプチェ風', 194, 218], // 第5弾のごま油でさらに上がった
    ['だしのとり方', 70, 139], // 第2弾のかつお節 15→25円/1袋 でさらに上がった
    ['豆腐とわかめの味噌汁', 28, 28], // 第6弾でわかめを判断待ちに戻したので元に戻った
...
// 野菜は季節で動くので、**調べた日（2026-08-26）を各行のコメントに必ず書いてある**。
{
  ...
  eq('LF-16 にんにく1かけは8円（判断待ちで60円/1玉のまま）', lf4Yen('にんにく', '1', 'かけ'), 8)
  eq('LF-16 にんにく1片も同じ8円（書き方ちがい）', lf4Yen('にんにく', '1', '片'), 8)
...
// ③が書けない＝グレードを決めずに採ったということなので、そこで止まる。
{
  ...
  const lf6Mine = lf6Blocks.filter((b) => b.comment.includes('便LF'))
  ...
  // --- LF-21: 産地で割れる品は、値を動かさずに両方の実勢を書いてある（司令部の判断待ち） ---
  const lf6Entry = (name) => {
    const e = PRICE_DEFAULTS.find((d) => d.name === name)
    return e ? [e.pricePerUnit, e.unit] : null
  }
  ...
  for (const [name, price, unit] of [
    ...
    eq(`LF-21 「${name}」は判断待ちなので元の${price}円/${unit}のまま`, lf6Entry(name), [price, unit])
  }
  for (const name of ['乾燥わかめ', '乾燥芽ひじき', '切り干し大根', 'にんにく']) {
    eq(`LF-21 「${name}」の行に国産の並と輸入の並が両方書いてある`, /国産の並/.test(lf6Comment(name)) && /輸入(?:・PB)?の並/.test(lf6Comment(name)), true)
  }
  eq('LF-21 乾燥わかめとカットわかめは同じ値のまま', lf6Entry('乾燥わかめ'), lf6Entry('カットわかめ'))
}
