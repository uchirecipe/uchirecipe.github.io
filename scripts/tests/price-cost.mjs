...
// 原価が過大(サラダ油「適量」=1Lボトル400円)・過小(鶏むね肉「1枚」=100g分90円)の両方向に壊れていた ----------
{
  ...
  // (8) 同梱109品の合計原価のピン留め(この数字が動いたら按分の前提が変わったということ)
  {
    ...
    // 鮭・生鮭 120→280円/1切れ・塩鮭 150→230円/1切れ・米 60→130円/1合・合いびき肉 130→210円/100g。直したのは3件
    ...
    eq('同梱109品の概算食費の合計(便LF第10弾後。便LF第9弾後は45,450円/便LF第8弾後は38,932円/便LF第7弾後は38,147円/便LF第6弾後は39,124円/便LF第5弾後は40,051円/便LF第4弾後は39,777円/便LF第3弾後は38,121円/便LF第2弾後は37,802円/便LF第1弾後は37,015円/便KX後は35,814円/便KE後は35,826円/便JI後は37,951円/便JG後は37,934円/便BY修正前は48,377円)', grand, 47430)
    ...
    eq('鶏の照り焼き 1食あたり(便LFのピン留め解除で343→431円・修正前280円は鶏もも肉2枚が100g分の130円だった)', Math.round(estimateRecipeCost(teriyaki.ingredients, idx).total / teriyaki.servings), 431)
  }
}
...
// 「実際はこれより高くなります」という案内が事実と逆になっていた。
{
  ...
  eq('KE-3 つくねの照り焼きの1食は240円（便LFのピン留め解除で194→240円・便KEの直後は186円・直す前は386円）', kePerServing(tsukune, 2), 240)
...
//   ・鶏むね肉 90円/100g → 実勢の中央値 106円/100g（差は+18%で、ほかの4件より小さい）
{
  ...
  for (const [title, before, after] of [
    ['ひじきの煮物', 50, 115], // 第6弾でひじきを判断待ちに戻したので、油揚げのぶんだけ残った
    ['きゅうりとわかめの酢の物', 58, 110], // 第6弾でわかめを判断待ちに戻したので元に戻った
    ['切り干し大根のハリハリ漬け', 67, 71], // 第6弾で判断待ちに戻したので元に戻った
    ['しらたきのチャプチェ風', 194, 253], // 第5弾のごま油でさらに上がった
  ...
  // （増えても減っても赤になるので、直したら一緒にここも直す）。
  {
    ...
    const LF_NO_SOURCE_KNOWN = [
      ...
      'スパゲッティ', '冷凍うどん', '食パン', '揚げ油', 'だし汁', '水またはだし汁', '中濃ソース', 'ケチャップ',
      'マヨネーズ', 'ポン酢', 'めんつゆ', 'カレールー', 'シチュールー', '鶏がらスープの素', 'おろしにんにく', '塩',
      '塩こしょう', 'こしょう', '七味唐辛子', '砂糖', '甜麺醤', '豆板醤', '粉山椒', 'ラー油',
      '紅しょうが', '刻みのり', '白ごま', '白すりごま', 'すりごま', '黒いりごま', 'いりごま', '白練りごま',
      'カットトマト缶', 'みかん缶', 'メープルシロップ', '黒みつ', 'アーモンドエッセンス', 'さわら', '万能ねぎ', '梅干し',
...
// 1件ずつの店名・商品名・容量・税込価格・調べた日は src/data/priceDefaults.ts の各行のコメント。
{
  ...
  for (const [name, pricePerUnit, unit] of [
    ...
    ['しょうゆ', 400, '1L'], // PBの並160円と全国銘柄の並278円が1.9倍ちがう＝司令部の判断待ち
