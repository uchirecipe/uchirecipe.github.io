...
import { safetyNotesFor, stepSafetyNotes, wholeRecipeSafetyNotes } from '../../src/logic/safetyNotes.ts'
{
  ...
  // --- JH-7: 文言の作法（D-④: 簡潔な常体1〜2文・必須は「〜こと」・菌名や恐怖をあおる語は書かない） ---
  {
    ...
    // 2026-08-28 便MC: 生・半生の3つ（鶏とレバー・豚・しめさば）を足したので下限も上げる。
    // 下限で見るのは「見張りが空振りしていないこと」で、決まりの数を画面に出す話ではない
    eq('JH-7 文言を読めている（0件なら見張りが壊れている）', jhTexts.length >= 9, true)
    const jhStyle = []
  ...
  // --- JH-9〜JH-11（2026-08-28 便MC）: 生・半生で食べる料理の3つ ---
  // オーナー原文（足すかどうかの問いに）「足してください」。
  // 足す3つは**どれも料理名を名指しで取る**。「鶏」＋「生」のような組み合わせで取ると、
  // 鶏を使うレシピすべてに注意が出て**毎回出る注意になり、本当に効かせたいものまで効かなくなる**。
  // ここで見張るのは「出る形」と「出してはいけない形」の両方。

  // --- JH-9: 鶏・レバーの生／半生（カンピロバクター） ---
  eq(
    'JH-9 鶏の生・半生を名乗る料理名には、レシピ全体に注意が付く',
    ['鶏刺し', 'とりわさ', '鶏のたたき', 'レバ刺し', '生レバー'].map((t) =>
      jhRules(jhRecipe(t, ['鶏むね肉'], ['薄く切って盛り付ける'])),
    ),
    [['rawChickenLiver'], ['rawChickenLiver'], ['rawChickenLiver'], ['rawChickenLiver'], ['rawChickenLiver']],
  )
  eq(
    'JH-9 材料名や手順に書かれていても拾う（料理名だけを見ているのではない）',
    jhRules(jhRecipe('晩ごはん', ['鶏むね肉'], ['鶏のたたきにして盛り付ける'])),
    ['rawChickenLiver'],
  )
  eq(
    'JH-9 普通に火を通す鶏料理には出さない（鶏を使うだけでは出さない）',
    [
      jhRecipe('鶏の唐揚げ', ['鶏もも肉'], ['鶏もも肉に下味をつけて揚げる']),
      jhRecipe('親子丼', ['鶏もも肉', '卵'], ['鶏もも肉を煮て、溶き卵でとじる']),
      jhRecipe('レバーの甘辛煮', ['鶏レバー'], ['鶏レバーを下ゆでしてから煮る']),
    ].map(jhRules),
    [[], [], []],
  )
  eq(
    'JH-9 鶏と関係のない「たたき」には出さない（かつお・きゅうり・梅）',
    [
      jhRecipe('かつおのたたき', ['かつお'], ['薄く切って盛り付ける']),
      jhRecipe('たたききゅうりの中華あえ', ['きゅうり'], ['きゅうりをたたいて割る']),
      jhRecipe('鶏むね肉とたたききゅうりのあえ物', ['鶏むね肉', 'きゅうり'], ['きゅうりをたたいて、ゆでた鶏むね肉とあえる']),
    ].map(jhRules),
    [[], [], []],
  )
  eq(
    'JH-9 「中まで火を通す」と書いてあるたたき風には出さない（生・半生で食べる品ではない）',
    jhRules(jhRecipe('鶏のたたき風サラダ', ['鶏むね肉'], ['鶏むね肉は中まで火を通してから冷やし、薄く切る'])),
    [],
  )
  eq(
    'JH-9 「表面だけ火を通す」では消さない（いちばん注意が要るレシピで出なくなるため）',
    jhRules(jhRecipe('鶏のたたき', ['鶏むね肉'], ['表面だけ火を通し、中は生のまま薄く切る'])),
    ['rawChickenLiver'],
  )

  // --- JH-10: 豚の生／半生（生食は国が禁じている） ---
  eq(
    'JH-10 豚の生・半生を名乗る料理名には、レシピ全体に注意が付く',
    ['豚刺し', '豚のたたき', 'レアポーク'].map((t) => jhRules(jhRecipe(t, ['豚ロース肉'], ['薄く切って盛り付ける']))),
    [['rawPork'], ['rawPork'], ['rawPork']],
  )
  eq(
    'JH-10 普通に火を通す豚料理には出さない',
    [
      jhRecipe('豚の生姜焼き', ['豚ロース肉'], ['豚ロース肉を焼いてたれをからめる']),
      jhRecipe('回鍋肉', ['豚バラ肉', 'キャベツ'], ['生の豚肉を切ったまな板はよく洗う']),
      jhRecipe('生ハムとメロン', ['生ハム', 'メロン'], ['メロンに生ハムをのせる']),
    ].map(jhRules),
    [[], [], []],
  )
  eq(
    'JH-10 「しっかり火を通す」と書いてあれば出さない',
    jhRules(jhRecipe('豚のたたき風', ['豚ロース肉'], ['豚ロース肉はしっかり火を通してから薄く切る'])),
    [],
  )

  // --- JH-11: しめさば（酢では魚の寄生虫は死なない） ---
  eq(
    'JH-11 しめさばを使うレシピには、レシピ全体に注意が付く',
    ['しめさば', '〆さば', '締め鯖'].map((n) => jhRules(jhRecipe('棒ずし', [n, '米'], ['ご飯の上にのせて巻く']))),
    [['shimesaba'], ['shimesaba'], ['shimesaba']],
  )
  eq(
    'JH-11 「刺身」全体には広げない（司令部の裁定。毎回出る注意にしない）',
    [
      jhRecipe('刺身の盛り合わせ', ['まぐろの刺身', 'たいの刺身'], ['皿に盛り付ける']),
      jhRecipe('さばの味噌煮', ['さば'], ['さばを煮る']),
      jhRecipe('さばの塩焼き', ['さば'], ['さばを焼く']),
    ].map(jhRules),
    [[], [], []],
  )
  eq(
    'JH-11 さばを一度冷凍すると同じ文に書いてあれば、二重に言わない',
    jhRules(jhRecipe('しめさば', ['さば'], ['さばは一度冷凍してから酢でしめる'])),
    [],
  )
  eq(
    'JH-11 「冷凍庫で保存」のような別の話では消さない（さばを冷凍した話ではない）',
    jhRules(jhRecipe('しめさばの押しずし', ['しめさば', '米'], ['ご飯にのせて形を作る', '残ったら冷凍庫で保存できる'])),
    ['shimesaba'],
  )
  ...
  // 表を広げて品が増えたら、その1件ずつが本当に必要かを確かめること（理由なしに増やさない）。
  {
    ...
    // 2026-08-28 便MC: 足した3つ（生・半生）は同梱109品で**1品も出ない**。
    // 上の一覧でも分かるが、広げすぎたときに「どの決まりが広がったのか」が名前で出るように分けておく
    eq(
      'JH-8(便MC) 生・半生の3つは同梱109品で1品も出ない（広げすぎの見張り）',
      starterDefs
        .flatMap((r) => safetyNotesFor(r).map((n) => ({ title: r.title, rule: n.rule })))
        .filter((x) => ['rawChickenLiver', 'rawPork', 'shimesaba'].includes(x.rule))
        .map((x) => `${x.title}:${x.rule}`),
      [],
    )
  }
}
