...
// ==========================================================================================
{
  ...
  const ML_RULES = [
    ...
    {
      語: '対応しています',
      直し方: '保証を名乗る言葉なので使わない（docs/76 §4・便MCの裁定）',
      見つけ方: String.raw`対応しています`,
      見本: ['多くのレシピサイトに対応しています'],
      見本外: ['多くのレシピサイトのページから取り込めます', '振動に対応した端末では振動します'],
    },
    {
    ...
    {
      // docs/76 §3: 選ぶ＝動詞／選択＝名詞・複合語。サ変動詞の「選択する」だけを止める
      語: '選択する（動詞）',
    ...
    {
      // docs/76 §1:「買い物」「荷物」は実体なので漢字。ひらがなで終わる実体の名詞を外す
      語: 'もの（物）',
      直し方: '形式的な語はひらがな（docs/76 §1）。実体を指す名詞（買い物・荷物）は漢字のまま',
      見つけ方: String.raw`(?<!買い|食べ|飲み|乗り|洗い|忘れ|落とし|落し|入れ|持ち|贈り|送り|借り|編み|履き|巻き|練り|干し|漬け|和え|炒め|揚げ|蒸し|焼き|煮|作り|縫い|置き|敷き|建て)(?<=[ぁ-ん])物`,
      見本: ['作った物を並べます'],
      見本外: ['買い物メモ', 'サラダ・和え物', '炒め物・煮物', '飲み物', '持ち物'],
    },
    {
      語: 'ため（為）',
    ...
    {
      // 「最後」「今後」「直後」「今後」「お支払い後」は別の読み（ご）なので当たらない形にする
      語: 'あと（後）',
      直し方: 'ひらがな（docs/76 §1）。「最後」「今後」「決済完了後」など「ご」と読む語は漢字のまま',
      見つけ方: String.raw`後(?=から|[0-9０-９]|\{)`,
      見本: ['後から直せます', 'お試しで使ってみる（後3回）'],
      見本外: ['最後の手順', '今後表示しない', 'お支払い後に表示されます', '押した直後だけ', '料理名の後ろに'],
    },
    {
      語: 'また（又）',
      直し方: 'ひらがな（docs/76 §1）',
    ...
    {
      // 「〜してみる」は補助動詞なのでひらがなのまま（規約H-2:補助的な語はひらがな）。
      // 「読み込みます」「絞り込みます」「進みます」のような別の動詞の連用形も当たらない
      語: 'みる（見る）',
      直し方: '本動詞は漢字（docs/76 §2）。「〜してみる」は補助動詞なのでひらがなのまま',
