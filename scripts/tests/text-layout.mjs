...
// 直す先は scripts/gen-food-price-page.mjs だが、出来上がりの文字を測る）。
{
  ...
  //   ・「あと」（あとから・あとで・あとにする）も見張らない … 2026-08-26 便LIの実測で、
  //     ja.ts の文言に出る単独の「あと」は**35件すべてかな書き**、漢字の「後」は
  //     最後・前後・後ろ・今後の4語（別の語）だけだった＝**割れていない**。オーナー指示
  //     （書き溜め0826）も「『あとにする』→『後にする』。**ひらがなで統一だったらそのままで
  //     問題なし**」という条件つきだったので、かな書きのまま据え置いた。「めやす」（漢字16件・
  //     かな4件で割れていた）と違い、ここを漢字に倒すと**同じ語が2つのつづり**になる
...
// 「米 60円/1合」＝生の米の値段に当たっていた。
{
  ...
  // 2026-08-26 便LF: 米を60→130円/1合（並の実勢）にしたので60→130円。
  // 便KWが見張っているのは「米油などが『米』の行に流れ込まないこと」で、米の値そのものではない
  eq('KW-4 「米」そのものは米の行から出る（2026-08-26 便LFで130円/1合。前は60円）', kwYen('米', '1', '合'), 130)
...
// マスタ側の身元が分からない組は認めない。詳しくは src/logic/priceEstimate.ts。
{
  ...
  // 2026-08-26 便LF: 酢を340→230円/1L（並のグレード＝穀物酢）にしたので10→7円。
  // この節が見張っているのは「調味酢が酢の行に当たること」で、値そのものではない
  eq('KX-2 「調味酢 大さじ2」は酢の値段（影響範囲テストB。便LFの前は10円）', kxYen('調味酢', '2', '大さじ'), 7)
...
// 正しく当たっていた119件と**オーナーの実データの38件**が落ちる）。理由は報告に書いた。
{
  ...
  // 価格マスタに項目が無く「価格が分からない材料」になっていた。家庭用500mlの実売でそろえて足した
  {
    ...
    // 2026-08-26 便LF: 並のグレード＝穀物酢でそろえ直して340→230円/1L。便KYが見張っているのは
    // 「ワインビネガーが酢とは別の行になっていること」で、酢の値そのものではない
    eq('KY-5 普通の酢は酢の行のまま（2026-08-26 便LFで230円/1L・大さじ2で7円）', [kyHit('酢'), kyYen('酢', '2', '大さじ')], ['酢', 7])
    // 版番号を上げないと、新しい行が既存の端末に届かない
...
// ==========================================================================================
// ML-1〜ML-7（2026-08-29 便ML）: docs/76_表記の基準.md を、人ではなく機械が守る
//
// なぜ要るか: 基準を作った当日だけで、便が3本とも別々の場所で基準を割った
// （「足す」4件・「タップ」16件・「ふだん」8件・「〜ぶん」9件・「出ます」51件）。
// docs/76 §6 に「見張りを置くまでは人が見る」と書いてあるとおり、いままで人が見ていた。
//
// 見る先: src/i18n/ja.ts の**画面に出る文言**（コメントは対象外。ja オブジェクトの値だけを歩く）と、
// public/about/*.html（コラムも含む11枚）の**本文・図の題（figcaption）・図の説明（alt）**。
// alt を本文と同格に見るのは docs/76 4-4⑨の裁定。
//
// **いちばん難しいのは例外の扱い。**docs/76 には例外が13件あり、機械で一括に見ると
// 「書き出す」（50件）「取り出す」「出典」（5件）「新しいバージョンが出ていないか」まで
// 赤にしてしまう。ここでは例外を2通りに分けた:
//   ①**見つけ方そのものが当たらない形にする**（書き出す・取り出す・出典・バージョンが出る・
//     タブレット・「買い物」等の実体の名詞・「〜に加えて」・「〜してみる」・「最後」「今後」・
//     「たぶん」「じゅうぶん」）。当たらないことは **ML-3 が毎回「見本外」で実測**している
//   ②**1件ずつ理由を書いて一覧に持つ**（scripts/data/ja-notation-known.json）。
//     docs/76 の裁定（算数の足し算・画面のボタン名・ブラウザの言葉）に当たるもの
//
// 「いま在るものを全部一覧に入れて0件にする」形にはしない。**JM-2 と同じ作り**で、
// 一覧より増えたら赤・減ったのに一覧が古いままでも赤にする（＝増やせないし、放置もできない）。
//
// **見張り自身が空振りしていないかの検査**（ML-2）を必ず置く。表記の見張りは
// 「1件も見つからない＝合格」に倒れやすく、正規表現を1文字壊しただけで永久に緑になる。
// そこで規則ごとに**わざと基準を割った見本**を持たせ、それを捕まえられなければ落とす。
//
// 禁じ手②（文言の書き写し）を踏まないための約束: 一覧の鍵は**文そのものではなく「場所と語」**
// （ja.ts は文言のキーの道すじ、ページはファイル名）。docs/76 4-5 のとおり、語尾まで写した見張りは
// 表記をそろえる作業そのものを妨げるので、文は一覧に持たない。
//
// docs/76 4-6 の裁定（2026-08-29）で決まったこと:
//   ⑭「あと」をひらがなにするのは「後で・後から」の意味のときだけ ⑮「〜してみる」は補助動詞
//   ⑯実体を指す「◯◯物」は漢字 ⑰見る先は説明ページ11枚すべて（コラム・法務文書も含む）。
//     ただし **foods.html は生成物なので、直す先は scripts/gen-food-price-page.mjs**
//     （出来上がりの字をここで測り、直すのは生成元。生成元だけ直しても `npm run gen:foods` を
//     忘れると本番に出ないので、測るのは生成物のほうにしてある）
//   ⑱「対応しています」を止めるのは保証を名乗る用法だけ ⑲「選択した◯◯」は名詞句なので止めない
//   ⑳**「消す」の例外④（画面がその語を使っているとき）は機械で判定しない**——
//     実測で説明ページの21件を「画面に同じ言い方があるか」で測ると9件が赤になるが、読むとどれも
//     正しく `ja.ts` と語順が違うだけだった（説明書「今のデータを消して」／画面「今のデータを
//     **全部**消して」）。**ここに「消す」の規則を足さないこと。**人が読む欄（docs/76 §7）に残す
// ==========================================================================================
{
  const mlRoot = path.join(path.dirname(fileURLToPath(scriptFileUrl)), '..')

  // ---- 見る先を集める。1つの「かたまり」＝場所（どこの文か）＋文字 ----
  const mlUnits = []
  {
    const walkJa = (obj, prefix) => {
      for (const [key, value] of Object.entries(obj)) {
        const full = prefix ? `${prefix}.${key}` : key
        if (typeof value === 'string') mlUnits.push({ where: `ja.ts:${full}`, text: value })
        else if (value && typeof value === 'object') walkJa(value, full)
      }
    }
    walkJa(ja, '')

    const mlBlank = (s) => s.replace(/[^\n]/g, ' ')
    // 段落・箇条書き・表のます目・図の題の区切りで切る（<strong> のような文の途中に入る印では切らない）
    const ML_BLOCK =
      /<\/?(?:p|li|h[1-6]|td|th|div|section|ul|ol|figure|figcaption|br|blockquote|table|tr|dl|dt|dd|details|summary|header|footer|main|nav|article|aside|option)\b[^>]*>/gi
    const aboutDir = path.join(mlRoot, 'public/about')
    const mlPages = []
    for (const e of readdirSync(aboutDir, { withFileTypes: true }).sort((a, b) => (a.name < b.name ? -1 : 1))) {
      if (e.isDirectory())
        for (const f of readdirSync(path.join(aboutDir, e.name)).filter((x) => x.endsWith('.html')).sort())
          mlPages.push(`${e.name}/${f}`)
      else if (e.name.endsWith('.html')) mlPages.push(e.name)
    }
    for (const f of mlPages) {
      const where = `public/about/${f}`
      // <script>/<style>/コメントは利用者が読まないので落とす（直した経緯をコメントに書き残せる）
      const body = readFileSync(path.join(aboutDir, f), 'utf-8')
        .replace(/<script[\s\S]*?<\/script>/gi, mlBlank)
        .replace(/<style[\s\S]*?<\/style>/gi, mlBlank)
        .replace(/<!--[\s\S]*?-->/g, mlBlank)
      // 図の説明（alt）も本文と同格に測る（docs/76 4-4⑨）
      for (const m of body.matchAll(/\balt="([^"]*)"/g)) mlUnits.push({ where, text: m[1] })
      for (const line of body.replace(ML_BLOCK, '\n').replace(/<[^>]+>/g, '').split('\n')) {
        const t = line.replace(/&[a-z]+;/g, ' ').replace(/\s+/g, ' ').trim()
        if (t && /[ぁ-んァ-ヶ一-龥]/.test(t)) mlUnits.push({ where, text: t })
      }
    }
  ...
  // ---- 規則。docs/76 の表をそのまま1行ずつ写したもの ----
  //
  // 「見本」＝わざと基準を割った文。ML-2 が「捕まえられること」を毎回測る（空振り防止）。
  // 「見本外」＝**書いてよい**言い方。ML-3 が「1つも赤くならないこと」を毎回測る（誤爆防止）。
  //   docs/76 4-3⑦の「触ってはいけない出る」（書き出す・取り出す・出典・新しいバージョンが出る）は
  //   ここに全部並べてある＝**誤爆しないことが検査の中に常駐している**。
  const ML_RULES = [
    // ---- 使わない語（docs/76 §4）----
    {
      語: 'タブ',
      直し方: 'アプリの中の呼び名には使わない。画面の名前で言う（docs/76 §4・規約H-2）',
      見つけ方: String.raw`タブ(?!レット)`,
      見本: ['週タブを開きます'],
      見本外: ['タブレットでも同じ画面です'],
    },
    {
      語: 'タップ',
      直し方: '「押す」にそろえる（マウスでも使えるため。docs/76 §4）',
      見つけ方: String.raw`タップ`,
      見本: ['日付をタップします'],
      見本外: ['日付を押します'],
    },
    {
      // docs/76 4-6⑱: 止めるのは**保証を名乗る用法だけ**。「振動に対応した端末では」は
      // **機器の対応**を言っているだけなので別物＝止めない（「対応した」の形は当たらない）
      語: '対応しています',
      直し方: '保証を名乗る言葉なので使わない（docs/76 §4・4-6⑱）',
      見つけ方: String.raw`対応しています`,
      見本: ['多くのレシピサイトに対応しています'],
      見本外: [
        '多くのレシピサイトのページから取り込めます',
        '振動に対応した端末では音の代わりに振動します',
        '対応しているブラウザなら写真も残ります',
      ],
    },
    {
      // docs/76 4-3⑦: 直してよいのは**画面に現れるという意味**のときだけ。
      // 「出す」の活用（書き出す・取り出す・書き出した・取り出せます）は、そもそも
      // 「出る」の活用と字が重ならないので当たらない。「出典」も後ろが「典」なので当たらない。
      // 「新しいバージョンが出る」（＝公開される意味）だけは字が同じなので、
      // 同じ文に「バージョン」があるかで外す（文ではなく**語**で見るので、書き直しても外れない）。
      語: '出る（表示の意味）',
      直し方: '「表示します」にそろえる（docs/76 §4・4-2①。アプリが主語なら能動＝4-2②）',
      見つけ方: String.raw`出(?:ます|ません|ています|ている|ていない|ていません|ない|る|た|て|れば)`,
      文の除外: String.raw`バージョン`,
      見本: ['メモがあれば一緒に出ます', '押している間だけ出るボタンです'],
      見本外: [
        'レシピをファイルに書き出す',
        '書き出した記録を端末から消す',
        '冷凍庫から取り出す',
        '出典: 日本食品標準成分表2020年版',
        '新しいバージョンが出ていないか、今すぐ確かめたいとき',
        '新しいバージョンが出たときは、アプリを開き直したときに入れ替わります',
      ],
    },
    {
      // 2026-09-02 便NA: 在庫の下がり方は「ある→少ない→ない」の順に「1つ下がる／1つ下げる」と言う。
      // 「1段階」だけでは何段階のうちの1つか分からない（2026-07-29 便CI/C06でペルソナ5体が指摘）。
      // 2026-09-02 に便MZが最後の1件を掃いて全廃したので、戻ってこないようにここで見張る。
      // 目盛りの数そのものを言う「3段階」（在庫は3段階で記録します）は別物なので当たらない
      語: '1段階',
      直し方: '在庫の下がり方は「ある→少ない→ない」の順に「1つ下がる」と言う（docs/76 §4・2026-09-02）',
      見つけ方: String.raw`[1１一]段階`,
      見本: ['使った食材の在庫を1段階下げます'],
      見本外: [
        '「ある→少ない→ない」の3段階で記録します',
        '使った食材の在庫が「ある→少ない→ない」の順に1つ下がります',
      ],
    },
    // ---- 言葉選び（docs/76 §3）----
    {
      // 「足りない」（足り）・「不足」「満足」「補足」は別の語なので当たらない。
      // 算数の足し算と画面のボタン名（docs/76 4-2③・4-4⑬）は一覧で1件ずつ許す。
      語: '足す',
      直し方: '品物や行を増やすなら「追加」（docs/76 §3）。算数の足し算と画面のボタン名は例外',
      見つけ方: String.raw`(?<![不満補])足(?=す|し[てたま]|せ|そ)`,
      見本: ['行を足せます'],
      見本外: ['材料が足りないときは', '睡眠不足でも作れます', '鶏の足を使います'],
    },
    {
      // 「〜に加えて」（添加・並列の副詞句）は「増やす」の話ではないので当たらない形にする
      語: '加える',
      直し方: '品物や行を増やすなら「追加」（docs/76 §3）',
      見つけ方: String.raw`(?<!に)加え`,
      見本: ['行を加えます'],
      見本外: ['材料・手順に加えて料理名も読み込みます', '加熱してから盛り付けます'],
    },
    {
      // docs/76 §3・4-6⑲: 選ぶ＝動詞／選択＝名詞・複合語。**「選択した◯◯」は名詞句なので直さない**——
      // 止めるのはサ変動詞の形（選択する／します／して）だけ
      語: '選択する（動詞）',
      直し方: '動詞は「選ぶ」。「選択」は名詞・複合語のときだけ（全選択・選択解除・選択中。docs/76 §3）',
      見つけ方: String.raw`選択(?:する|します|して|しま)`,
      見本: ['食材を選択してください'],
      見本外: ['選択した食材3件を削除', '全選択', '選択解除', '選択中'],
    },
    // ---- ひらがなにするもの（docs/76 §1）----
    {
      語: 'こと（事）',
      直し方: '形式的な語はひらがな（docs/76 §1）。「食事」「大事」のような実体は漢字のまま',
      見つけ方: String.raw`(?<=[ぁ-ん])事`,
      見本: ['できる事を増やします'],
      見本外: ['食事の記録', '大事な設定', '返事を待ちます'],
    },
    {
      語: 'とき（時）',
      直し方: '形式的な語はひらがな（docs/76 §1）。「時間」「時短」のような熟語は漢字のまま',
      見つけ方: String.raw`(?<=[ぁ-ん])時(?![一-龥])`,
      見本: ['作る時に押します'],
      見本外: ['調理時間', '食事の時間', '時短の工夫'],
    },
    {
      // docs/76 §1・4-6⑯: 実体を指す「◯◯物」は漢字——買い物・荷物に加えて
      // **和え物・炒め物・煮物・飲み物・持ち物**も同じ。ひらがなで終わる実体の名詞をここで外す
      //（「煮物」のように漢字で終わるものは、手前がひらがなでないので最初から当たらない）
      語: 'もの（物）',
      直し方: '形式的な語はひらがな（docs/76 §1）。実体を指す名詞は漢字のまま（docs/76 4-6⑯）',
      見つけ方: String.raw`(?<!買い|食べ|飲み|乗り|洗い|忘れ|落とし|落し|入れ|持ち|贈り|送り|借り|編み|履き|巻き|練り|干し|漬け|和え|炒め|揚げ|蒸し|焼き|煮|作り|縫い|置き|敷き|建て)(?<=[ぁ-ん])物`,
      見本: ['作った物を並べます'],
      見本外: ['買い物メモ', '荷物', 'サラダ・和え物', '炒め物・煮物', '飲み物', '持ち物'],
    },
    {
      語: 'ため（為）',
      直し方: '形式的な語はひらがな（docs/76 §1）',
      見つけ方: String.raw`(?<![一-龥])為`,
      見本: ['見やすくする為に'],
      見本外: ['正常な運営を妨げる行為'],
    },
    {
      語: 'ください（下さい）',
      直し方: 'ひらがな（docs/76 §1・規約H-2）',
      見つけ方: String.raw`下さい`,
      見本: ['押して下さい'],
      見本外: ['押してください'],
    },
    {
      語: 'できる（出来）',
      直し方: 'ひらがな（docs/76 §1）',
      見つけ方: String.raw`出来(?!事)`,
      見本: ['登録が出来ます'],
      見本外: ['登録できます', 'ファイルに書き出せます'],
    },
    {
      語: 'いる（居る）',
      直し方: 'ひらがな（docs/76 §1）',
      見つけ方: String.raw`(?<![住入芝鳥新])居(?=る|ま|な|た|て)`,
      見本: ['登録して居ます'],
      見本外: ['住居', '新居', '居酒屋'],
    },
    {
      語: 'すべて（全て）',
      直し方: 'ひらがな（docs/76 §1。実測 すべて22／全て1）',
      見つけ方: String.raw`全て`,
      見本: ['全て作った！'],
      見本外: ['すべて作った！', '全選択'],
    },
    {
      // docs/76 4-6⑭: ひらがなにするのは**「後で・後から」の意味のときだけ**。
      // 実測（便ML）で `ja.ts` の漢字の「後」34件は**34件とも「ご」読み**（最後・今後・直後・
      // 決済完了後・後ろ）＝読み分けを書かずに機械にかけると34件とも誤爆する。
      // 「後です」「後でした」は「ご」読みの言い切りなので、うしろが「す・し・ょ」なら外す
      語: 'あと（後）',
      直し方: 'ひらがな（docs/76 §1・4-6⑭）。「最後」「今後」「決済完了後」など「ご」と読む語は漢字のまま',
      見つけ方: String.raw`後(?=から|で(?![すしょ])|[0-9０-９]|\{)`,
      見本: ['後から直せます', '一度開いた後であれば', 'お試しで使ってみる（後3回）'],
      見本外: [
        '最後の手順',
        '今後表示しない',
        'お支払い後に表示されます',
    ...
    // ---- 漢字にするもの（docs/76 §2・規約H-2）----
    {
      語: 'つくる（作る）',
      直し方: '意味を担う語は漢字（docs/76 §2）',
      見つけ方: String.raw`つく(?:る|ります|った|って|り方|りましょう)`,
      見本: ['献立をつくる'],
      見本外: ['献立を作る', '丸いボタンを押すと色がつきます'],
    },
    {
      // docs/76 4-6⑮: **「〜してみる」は補助動詞なのでひらがな**（規約H-2）。
      // 実測（便ML）で「み」を含む26件は26件とも別動詞の連用形（読み込みます・絞り込みます・
      // 進みます）か補助動詞（使ってみる）だった＝手前の字で本動詞だけに絞る
      語: 'みる（見る）',
      直し方: '本動詞は漢字（docs/76 §2）。「〜してみる」は補助動詞なのでひらがなのまま（4-6⑮）',
      見つけ方: String.raw`(?<=[をがはにもへ、。「])み(?:る|ます|ました|て|ましょう)`,
      見本: ['味をみる'],
      見本外: ['お試しで使ってみる', '材料と手順を読み込みます', '候補を絞り込みます', '調理中モードが進みます'],
    },
    {
      語: 'まえ（前）',
      直し方: '意味を担う語は漢字（docs/76 §2）',
      見つけ方: String.raw`(?<![しじ])まえ(?=[にのはがでを、。])`,
      見本: ['作るまえに確かめます'],
      見本外: ['先に決めてしまえば', '作る前に確かめます'],
    },
    {
      語: '買い物',
      直し方: '実体を指す名詞は漢字（docs/76 §2。実測 買い物43／0）',
      見つけ方: String.raw`買いもの|かいもの`,
      見本: ['買いものメモ'],
      見本外: ['買い物メモ'],
    },
    {
      語: '普段',
      直し方: '漢字（docs/76 §2。2026-08-28 オーナー指摘「ふだん」→「普段」）',
      見つけ方: String.raw`ふだん`,
      見本: ['ふだんよく買う商品'],
      見本外: ['普段よく買う商品'],
    },
    {
      // 「たぶん」「じゅうぶん」「ずいぶん」は別の語。「登録したぶん」（＝〜した分）は残す必要があるので、
      // 「た」の手前がひらがなでないときだけ副詞の「たぶん」とみなして外す
      語: 'ぶん（分と書く）',
      直し方: '人数・数量の「分」も、副詞的な「その分」も漢字（docs/76 §2・4-3⑥）',
      見つけ方: String.raw`(?<!じゅう|ずい|いく|なん|はん)(?<!(?:^|[^ぁ-ん])た)ぶん`,
      見本: ['1日ぶんのカード', '登録したぶんは残りません'],
      見本外: ['1日分のカード', 'たぶん大丈夫です', 'じゅうぶんな量', '登録した分は残りません'],
    },
  ]

  /** 1つの規則を1つの文に当てて、当たった箇所を返す */
  const mlHits = (rule, text) => {
    const re = new RegExp(rule.見つけ方, 'g')
    const skip = rule.文の除外 ? new RegExp(rule.文の除外) : null
    if (skip && skip.test(text)) return []
    const out = []
    let m
    while ((m = re.exec(text))) {
      out.push(text.slice(Math.max(0, m.index - 12), m.index + 14))
      if (m[0].length === 0) re.lastIndex++ // 幅0の当たりで止まらないための保険
    }
    return out
  }

  // ---- ML-2: 見張りが空振りしていないか（規則ごとに、わざと割った見本を捕まえられること） ----
  // 表記の見張りは「1件も見つからない＝合格」に倒れやすい。正規表現を1文字壊すと永久に緑になるので、
  // **規則の数だけ見本を持たせて毎回当てる**。見本が捕まらなければ、その規則は死んでいる。
  eq('ML-2 前提: 規則が並んでいる（0件なら見張りが壊れている）', ML_RULES.length >= 20, true)
  eq(
    'ML-2 どの規則も、わざと基準を割った見本を捕まえられる（空振りしていない）',
    ML_RULES.flatMap((rule) =>
      (rule.見本 ?? []).filter((s) => mlHits(rule, s).length === 0).map((s) => `${rule.語}: 「${s}」を捕まえられない`),
    ),
    [],
  )
  eq(
    'ML-2 どの規則にも見本と見本外が書いてある（片方だけだと空振りか誤爆に気づけない）',
    ML_RULES.filter((r) => (r.見本 ?? []).length === 0 || (r.見本外 ?? []).length === 0).map((r) => r.語),
    [],
  )

  // ---- ML-3: 誤爆していないか（**書いてよい言い方**が1つも赤にならないこと） ----
  // docs/76 4-3⑦の「触ってはいけない出る」（書き出す・取り出す・出典・新しいバージョンが出る）も
  // ここに入っている。**どの規則で見ても赤にならない**ことを測る（規則をまたいだ巻き込みも防ぐ）。
  {
    const mlSafe = ML_RULES.flatMap((r) => (r.見本外 ?? []).map((s) => ({ 語: r.語, 文: s })))
    eq('ML-3 前提: 書いてよい言い方の見本外が並んでいる（0件なら誤爆の検査になっていない）', mlSafe.length >= 40, true)
    eq(
      'ML-3 書いてよい言い方が、どの規則でも赤にならない（例外の語を書いても誤爆しない）',
      mlSafe.flatMap(({ 語, 文 }) =>
        ML_RULES.flatMap((rule) => mlHits(rule, 文).map((h) => `「${文}」（${語}の見本外）が規則「${rule.語}」に当たった: ${h}`)),
      ),
      [],
    )
  }

  // ---- ML-4〜ML-6: いま基準を割っている箇所を、一覧と突き合わせる（JM-2 と同じ作り） ----
  {
    const mlNow = {}
    const mlWhere = {}
    for (const unit of mlUnits) {
      for (const rule of ML_RULES) {
        const hits = mlHits(rule, unit.text)
        if (hits.length === 0) continue
        // 一覧の鍵は「場所 / 語」。ページはファイル名まで、ja.ts は文言のキーの道すじまで。
        // **文そのものは鍵にしない**（docs/76 4-5: 語尾まで写した見張りは、表記をそろえる作業を妨げる）
        const key = `${unit.where} / ${rule.語.replace(/（.*/, '')}`
        mlNow[key] = (mlNow[key] ?? 0) + hits.length
        ;(mlWhere[key] ??= []).push(...hits)
      }
    }

    const mlKnownRaw = JSON.parse(readFileSync(path.join(mlRoot, 'scripts/data/ja-notation-known.json'), 'utf-8'))
    const mlKnown = {}
    for (const box of ['例外', '積み残し'])
      for (const [key, v] of Object.entries(mlKnownRaw[box] ?? {})) mlKnown[key] = (mlKnown[key] ?? 0) + v.数

    // ML-7: 一覧の1件ずつに理由が書いてあること（理由の無い行は「黙って許した」ことになる）
    eq(
      'ML-7 一覧の1件ずつに理由が書いてある（理由なしで例外にも積み残しにもしない）',
      ['例外', '積み残し'].flatMap((box) =>
        Object.entries(mlKnownRaw[box] ?? {})
          .filter(([, v]) => typeof v.理由 !== 'string' || [...v.理由].length < 20)
          .map(([key]) => `${box}「${key}」の理由が空か短すぎる`),
      ),
      [],
    )
    eq('ML-7 前提: 一覧を読めている（0件なら突き合わせが素通りする）', Object.keys(mlKnown).length > 10, true)
    // 例外が2件以上まとまっている行は、**1件ずつの理由**（内訳）を数だけ並べる。
    // まとめて1文の理由で通すと「同じ理由の箱」ができて、あとから中身が増えても気づけない
    eq(
      'ML-7 2件以上まとまっている例外に、1件ずつの理由（内訳）が数だけ並んでいる',
      Object.entries(mlKnownRaw['例外'] ?? {})
        .filter(([, v]) => v.数 >= 2 && (v.内訳 ?? []).length !== v.数)
        .map(([key, v]) => `例外「${key}」は${v.数}件なのに内訳が${(v.内訳 ?? []).length}行`),
      [],
    )

    const mlGrew = []
    const mlShrank = []
    for (const [key, n] of Object.entries(mlNow)) {
      const was = mlKnown[key] ?? 0
      if (n > was)
        mlGrew.push(
          `${key} が${was}→${n}件に増えた（docs/76 のとおり直すか、例外なら理由を書いて scripts/data/ja-notation-known.json に足すこと）例: ${mlWhere[key].slice(0, 2).join(' / ')}`,
        )
    }
    for (const [key, was] of Object.entries(mlKnown)) {
      const n = mlNow[key] ?? 0
      if (n < was) mlShrank.push(`${key} は${was}→${n}件に減った（一覧の数を下げるか、行ごと消してください）`)
    }
    eq('ML-4 表記の基準を割っている箇所が、一覧より増えていない', mlGrew, [])
    eq('ML-5 一覧に、もう割っていないものが残っていない（直したら数を下げる）', mlShrank, [])
    eq(
      'ML-6 割っている箇所の総数が一覧どおり（数え方が変わったら気づけるようにする）',
      Object.values(mlNow).reduce((a, b) => a + b, 0),
      Object.values(mlKnown).reduce((a, b) => a + b, 0),
    )
  }
}
