...
//
// --- 2026-08-29 便MQ が数え直した（司令部の裁定「切り出せない節をなくす」） ---
// **切り出せない節は 36 → 32**（`currentCheck = '…'` で名前が付いている節を数えた。
// 判定ラベルの名前まで数えると 78 → 74）。減った4つ（SETTINGS-TAB-01・BANNER-01・
// MOVEGUIDE-01・URLIMPORT-00 と、名前が currentCheck に無かった REFRESH-APP-01）は、
// **塊は在るのに `currentCheck` が無かった11個**に名札を足して選べるようにしたぶん。
// **残り32節は「塊が1つも無い＝丸ごと直書き」**（01・02・04・05・08 の頭のほう。SMK-01・
// COUNT-01・PRICE-01・TIMER-ADJ-01 など）で、**`currentCheck` を足しても選べない**
// ＝選べるようにするには `{ }` で囲うしかなく、節をまたいで使っている `const` が見えなくなる。
// **名札を足すだけでは足りないことも実測で分かっている**: SETTINGS-TAB-01 は名札を足した直後、
// 切り出せてはいるのに5件とも赤（設定を開く操作と、節の上端を読む道具が塊の外にあった）。
// **切り出して測れる形にするには、その節の「開く操作」を塊の中に置き、道具は _shared.mjs に置く。**
// ==========================================================================================
