...
// アトム列は ComposedStepText(TermText→TimeText)と同じ分解結果を手で並べたもの(用語/タイマー=箱)。
{
  ...
  // 罰則DP(切れ端は特大罰則)で「洗っていない / 米を③のフライパンに加えて、」に均し、切れ端を消す。
  const exA = () => [
  ...
  for (const w of [17, 18, 19]) {
    eq(`lineCompose 要件A 幅${w}(切れ端「加えて、」を作らない)`, compose(exA(), w), [
      '洗っていない',
      '米を③のフライパンに加えて、',
      '3分ほど透き通るまで炒めます。',
    ])
  ...
  // 要件F(実現可能な範囲): 「…みそだれを軽くぬぐった鮭の皮目を下にして焼く。」の後半。
  // オーナー希望は「みそだれを軽くぬぐった鮭の皮目を / 下にして焼く。」(16/7)だが、
  // BudouX/jaWrap が「鮭の皮目を下にして」を格助詞「を」結合で1つの分割不能ユニットにするため、
  // 「鮭の皮目を|下にして」で割ることは行組み側では不可能(jaWrap結合ロジックは変更禁止)。
  // よって到達可能な最良分割=「みそだれを軽くぬぐった / 鮭の皮目を下にして焼く。」(11/12)を固定する。
  // 罰則DPは発動しない(貪欲の末尾行12字>4字=切れ端でない)ので貪欲どおり。詳細は報告書F項参照。
  const exF2 = () => [txt('みそだれを軽くぬぐった鮭の皮目を下にして焼く。')]
  for (const w of [17, 18, 19]) {
    eq(`lineCompose 要件F後半 幅${w}(到達可能な最良=11/12)`, compose(exF2(), w), [
      'みそだれを軽くぬぐった',
      '鮭の皮目を下にして焼く。',
    ])
  }
  ...
  // 非退行: オーナー確認済み「こんにゃくの炒り煮」基準1の幅17/18は貪欲どおり(罰則DPで変えない)。
  // (基準1の期待は上のブロックで固定済み。ここでは罰則DPが誤発動していないことを再確認する)
  const exKon = () => [
    txt('鍋にたっぷりの湯を沸かし、こんにゃくを'),
    box('m0', '2分ほど'),
    box('t0', '下茹で'),
  ...
  eq('lineCompose 非退行 こんにゃく基準1 幅17', compose(exKon(), 17), [
    '鍋にたっぷりの湯を沸かし、',
    'こんにゃくを2分ほど下茹でして',
    'ざるにあげ、水気を切る。',
  ])
