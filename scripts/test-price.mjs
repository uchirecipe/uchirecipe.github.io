...
// (E2E PRICE-01が玉ねぎの目安価格に依存。データ整備で既存行の値を書き換えないためのピン留め。
//  2026-08-26 便LFで玉ねぎを50→77円/1個にしたので、e2e側(scripts/e2e/05-price-mealplan.mjs)の
//  期待値も一緒に直してある)。
...
// 2026-08-26 便LF: 牛こま切れ肉を200→253円/100gに更新した(オーナー裁定「産地は輸入の並で置く」)。
// **同じ食材の「牛薄切り肉」「牛切り落とし肉」と3行そろえて動かしている**——1行だけ動かすと
// 同じ肉が書き方で値段が変わるため。実測の帯(輸入の並253円・国産の並528円)・調べた店と商品・
// 調べた日(2026-08-26)は src/data/priceDefaults.ts の「牛薄切り肉」の行のコメント。
// **ピン留めの仕組みはここでも残す**（値が勝手に動かないための見張り）。
...
// 【2026-08-26 便LF・オーナー裁定「ピン留めを外して並の実勢へ」】
// 便LFが2店（ヤオコーネットスーパー／東急ストアネットスーパー）の**並のグレード**
// （各店の標準品を1つずつ。生芋・手造り・国産大豆・有機・産直・ブランド産地・上位等級・
//  銘柄鶏/地鶏・少量パックの割高品は入れない）で測り直した値に、20件を書き換えた。
// **1件ずつの店名・商品名・容量・税込価格・調べた日は src/data/priceDefaults.ts の各行のコメント**に
// 書いてある（ここと priceDefaults.ts の両方から辿れる形にしてある）。
//   玉ねぎ 50→77 ／ にんじん 40→91 ／ じゃがいも 40→77 ／ **キャベツ 130→75（下がる）** ／
//   白菜 150→214 ／ 大根 100→161 ／ もやし 30→37 ／ きゅうり 40→80 ／ トマト 60→155 ／
//   なす 50→72 ／ ねぎ 100→139 ／ ほうれん草 100→257 ／ しめじ 100→125 ／ えのき 80→120 ／
//   鶏もも肉 130→165 ／ 鶏むね肉 90→116 ／ 豚バラ肉 150→300 ／ 牛こま切れ肉 200→253 ／
//   卵 25→37 ／ 牛乳 200→257 ／ 豆腐 40→85
// **同じ食材が別名で並ぶ行は、まとめて動かしてある**（豆腐/木綿豆腐/絹ごし豆腐・豚バラ肉/豚バラ薄切り・
// 牛こま切れ肉/牛薄切り肉/牛切り落とし肉・ねぎ/長ねぎ）。1行だけ動かすと書き方で値段が変わるため。
// 豚ロース薄切り(180→278)も一緒に動かした——据え置くと「ロースがバラより安い」逆転が残る。
// **そのあと便LFが第10〜11弾で追加で測って動かした4件**（2026-08-26。根拠は priceDefaults.ts の各行）:
//   合いびき肉 130→210 ／ 鮭 120→280 ／ 米 60→130 ／ **しょうゆ 400→278**
// しょうゆは**全国銘柄の並**（2店ともキッコーマン1L）を採った＝司令部裁定。PBの並160円は行にも残してある。
// 選ぶ線は「安いほう」ではなく「**並＝普通に買われているもの**」で、わかめ・ひじきで輸入を採ったのと
// 同じ考え方（あちらは輸入が定番・国産が上のグレード／調味料はその逆で全国銘柄が定番）。
// **まだ測っていないので据え置いた3件**: 豚こま切れ肉・さば・ピーマン（ピーマンだけは測ったが単一ソース）。
// 次に測る便が並のグレードで測って直すこと。
//
// **2026-08-26 の事故**: 上の4件を動かしたときに**この一覧の更新を忘れ、`npm test` が赤のまま本番へ出した**。
// 司令部の確かめ方が `npm test | grep '^合格'` だったため、**`test-price.mjs` の `✗` 形式を拾えなかった**
// （`npm test` は `&&` 連結なので、ここで止まると後ろの2本も走らない）。
// **`npm test` は終了コードで見ること。**grep で合格行を探す形にしない。
// **みそ11円は据え置き**（2026-08-25 司令部裁定。便KWの実測9.9円との差は大さじ1あたり1円）。
// **ピン留めの仕組みそのものはここでも残す**——勝手に値が動かないための見張りなので、
// 実勢に合わせて動かすときだけ、根拠つきでこの数字を書き換える。
const ORIGINAL_30 = [
  ['玉ねぎ', 77, '1個'], ['にんじん', 91, '1本'], ['じゃがいも', 77, '1個'],
  ['キャベツ', 75, '1/4個'], ['白菜', 214, '1/4個'], ['大根', 161, '1/2本'],
  ['もやし', 37, '1袋'], ['きゅうり', 80, '1本'], ['トマト', 155, '1個'],
  ['ピーマン', 30, '1個'], ['なす', 72, '1本'], ['ねぎ', 139, '1本'],
  ['ほうれん草', 257, '1束'], ['しめじ', 125, '1パック'], ['えのき', 120, '1袋'],
  ['鶏もも肉', 165, '100g'], ['鶏むね肉', 116, '100g'], ['豚バラ肉', 300, '100g'],
  ['豚こま切れ肉', 110, '100g'], ['牛こま切れ肉', 253, '100g'], ['合いびき肉', 210, '100g'],
  ['鮭', 280, '1切れ'], ['さば', 100, '1切れ'],
  ['卵', 37, '1個'], ['牛乳', 257, '1L'], ['バター', 600, '200g'], ['豆腐', 85, '1丁'],
  ['米', 130, '1合'], ['しょうゆ', 278, '1L'], ['みそ', 11, '大さじ1'],
]
...
// 207→213件（2026-08-26 便LF: チーズ・クリームチーズ・カッテージチーズ・カマンベール・
//   モッツァレラ・マスカルポーネ。成分表には（チーズ類）15品が入っているのに価格マスタには
//   「ピザ用チーズ」「粉チーズ」しか無く、スライスチーズやクリームチーズが「価格が分からない材料」に
//   なっていた。便LDが1件ずつ出していた候補を、便LFがヤオコー／東急ストアの2店で裏を取って確定した。
//   マスカルポーネは便LDの申し送りには無いが、「チーズ」の行を足すと「マスカルポーネチーズ」が
//   末尾一致で「チーズ」の値段に当たってしまうので、自分の行を持たせた。
//   **チェダーは行を作っていない**——家庭用で「チェダー」と書かれた商品にはプロセスチーズが混ざり、
//   何の値段か定まらないため（司令部の指示）。1件ずつの根拠は priceDefaults.ts の各行のコメント）
const PRICE_DEFAULTS_COUNT = 213
check(
